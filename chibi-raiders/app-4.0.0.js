...
function heroPortrait(h,cls="heroPortrait",locked=false){
 ...
 if(!src||src==="url_placeholder.png")return '<span class="'+cls+' heroPortraitFallback">'+fallback+'</span>';
 return '<span class="'+cls+' heroPortraitImage"><img src="'+src+'" alt="'+h.name+'" loading="lazy" onerror="this.parentElement.classList.add(\'imgError\')"><i>'+fallback+'</i></span>';
}
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-4.0.0.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
