...
function renderLobbyIdentity(){
 const squad=$("#lobbySquad");if(squad){
  ...
  $$(".lobbySquadHero[data-hero]").forEach(b=>b.addEventListener("click",()=>{selectedHero=b.dataset.hero;renderHeroes();showScreen("herois")}));
 }
...
function bindStaticControls(){
 ...
 bindClick("quickCampaignTab",()=>showScreen("campanha"));bindClick("quickHeroesTab",()=>showScreen("herois"));bindClick("quickPortalTab",()=>showScreen("portal"));
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-4.5.0.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
