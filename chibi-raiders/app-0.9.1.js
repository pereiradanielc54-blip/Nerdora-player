...
/* PWA / atualização robusta — arquivos versionados */
let remoteVersionInfo=null;
function status(t){const e=$("#updateStatus");if(e)e.textContent=t}
function hideSplash(ms=300){
 setTimeout(()=>{
  const x=$("#updateSplash");
  if(x){x.classList.add("hidden");setTimeout(()=>x.remove(),350)}
 },ms);
}
function timeout(ms){return new Promise((_,rej)=>setTimeout(()=>rej(new Error("timeout")),ms))}
async function fetchVersion(){
 const req=fetch("./version.json?ts="+Date.now(),{cache:"no-store"});
 const res=await Promise.race([req,timeout(2500)]);
...
async function checkUpdate(showMsg=false){
 ...
 try{
  const v=await fetchVersion();remoteVersionInfo=v;
  if(v.version&&v.version!==APP_VERSION){
   status("Nova versão "+v.version+" disponível.");
   const b=$("#splashUpdateBtn");if(b)b.style.display="";
   if(n){
    n.innerHTML='Nova versão <b>'+v.version+'</b> disponível. Toque em “Verificar atualização” para instalar.';
    n.classList.add("show");
   }
   const chk=$("#checkUpdateBtn");if(chk)chk.textContent="⬆ Atualizar para v"+v.version;
   hideSplash(650);
   return true;
  }
  status("Nerdora Chibi Raiders v"+APP_VERSION+" está atualizado.");
  if(showMsg&&n){n.textContent="Você já está na versão "+APP_VERSION+".";n.classList.add("show")}
 }catch(e){
  status("v"+APP_VERSION+" pronta. A verificação online demorou demais.");
  if(showMsg&&n){n.textContent="Não foi possível consultar a atualização agora, mas o jogo pode ser usado normalmente.";n.classList.add("show")}
 }
 hideSplash(550);
 return false;
}
window.addEventListener("beforeinstallprompt",e=>{
 ...
 const a=$("#installAppBtn"),b=$("#splashInstallBtn");
 if(a)a.style.display="";if(b)b.style.display="";
});
async function install(){
 if(!deferred){
  const n=$("#homeUpdateNotice");
  if(n){n.textContent='Abra o menu do navegador e escolha “Instalar app” ou “Adicionar à tela inicial”.';n.classList.add("show")}
  return;
 }
 deferred.prompt();await deferred.userChoice;deferred=null;
}
async function updateNow(){
 status("Abrindo a nova versão…");
 try{
  const v=remoteVersionInfo||await fetchVersion();
  if(v.entry){
   location.replace(v.entry+(v.entry.includes("?")?"&":"?")+"update="+Date.now());
   return;
  }
 }catch(e){}
 location.reload();
...
if("serviceWorker" in navigator){
 addEventListener("load",()=>{
  navigator.serviceWorker.register("./sw-0.9.1.js",{scope:"./",updateViaCache:"none"})
   .then(r=>{reg=r;r.update().catch(()=>{})})
   .catch(()=>{});
  checkUpdate(false);
 });
}else addEventListener("load",()=>checkUpdate(false));

/* Nunca deixa a splash bloquear o jogo indefinidamente. */
setTimeout(()=>hideSplash(0),3500);

$("#installAppBtn").onclick=install;
$("#splashInstallBtn").onclick=install;
$("#checkUpdateBtn").onclick=async()=>{
 const has=await checkUpdate(true);
 if(has)updateNow();
};
$("#splashUpdateBtn").onclick=updateNow;
