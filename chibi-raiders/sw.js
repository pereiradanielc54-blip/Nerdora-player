const CACHE_NAME="nerdora-chibi-raiders-v0.9.0";
...
self.addEventListener("fetch",e=>{
 ...
 e.respondWith(caches.match(e.request).then(cached=>{
  const net=fetch(e.request).then(r=>{if(r&&r.ok){const x=r.clone();caches.open(CACHE_NAME).then(c=>c.put(e.request,x))}return r}).catch(()=>cached);
  return cached||net;
 }));
});