...
function defaultSave(){
 ...
 return{
  ...
  lastAfkClaim:Date.now(),audioMuted:false,storySeen:{},
...
let save=loadSave(),formation={...save.formation},activeSlot="F1",selectedHero="seraphina",selectedAwakeningHero="seraphina",battle=null,deferred=null,reg=null,arenaOpponents=[],lastBattleMode="campaign",afkTicker=null,storyState=null;
...
function show(id){
 ...
 const navMap={formationScreen:"synergy",heroesScreen:"heroes",shopScreen:"shop",missionsScreen:"missions"};
...
function openNav(key){
 const map={
  ...
  equipment:()=>{renderHeroes();show("heroesScreen");setBottomNavActive("equipment")},
  ...
  synergy:()=>{renderFormation();show("formationScreen");setBottomNavActive("synergy")}
 };
 if(map[key])map[key]();
}
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-2.0.1.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
