...
function defaultSave(){
 const heroLevels={},shards={},owned={},equipment={},stars={};
 HEROES.forEach(h=>{heroLevels[h.id]=1;shards[h.id]=0;owned[h.id]=false;equipment[h.id]=blankEquipment();stars[h.id]=1});
 ...
 return{
  ...
  towerFloor:1,towerBest:0,abyssCrystals:0,
  unlockedStage:1,selectedStage:1,heroLevels,shards,owned,equipment,stars,gearInventory:{},potions:0,
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-1.2.0.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
