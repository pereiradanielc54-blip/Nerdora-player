...
function buildHero(h){
 const magic=h.type==="magic",secondary=Math.round(h.atk*.28);
 return{
  ...h,
  role:h.class,
  line:h.position,
  pa:magic?secondary:h.atk,
  ma:magic?h.atk:secondary,
  pd:Math.round(h.def*(h.physDefMult||1)),
  md:Math.round(h.def*(h.magicDefMult||.94)),
  sp:h.speed,
  cr:h.cr??.08,
  dg:h.dg??.05
 };
...
function heroPortrait(h,cls="heroPortrait",locked=false){
 ...
 if(Number.isInteger(h?.atlasCol)&&Number.isInteger(h?.atlasRow)){
  ...
  return '<span class="'+cls+' heroPortraitAtlas" style="'+heroRarityStyle(h)+'--atlas-x:'+x+'%;--atlas-y:'+y+'%;background-image:url(\''+HERO_ATLAS_SRC+'\')" aria-label="'+h.name+'"></span>';
 }
...
function renderHeroDetail(focus="overview"){
 ...
 <section class="heroUpgradeGrid">
  ...
  <article class="heroUpgradeCard ultimateCard">
   ...
   <p>Potência base ×${(h.ult.ratio*rar.ultMult).toFixed(2)} • escala com raridade e Despertar.</p>
  </article>
 </section>
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-5.0.1.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
