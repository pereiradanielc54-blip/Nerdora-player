...
function renderHeroes(){
 const grid=$("#heroManageGrid");if(!grid)return;grid.innerHTML="";
 sortedHeroes().forEach(h=>{
  const own=!!save.owned[h.id],st=heroStats(h),e=document.createElement("button");
  e.className="heroManageCard heroCollectionCard "+heroRarityClass(h)+(selectedHero===h.id?" selected":"")+(!own?" locked":"");e.dataset.hero=h.id;
  ...
  e.addEventListener("click",()=>openHeroDetail(h.id));
  grid.appendChild(e);
 });
 renderResources();
}
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-5.1.0.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
setTimeout(()=>hideSplash(0),3500);

resetDailyMissions();dailyArenaReset();dailyWorldBossReset();dailyGuildWarReset();renderResources();renderVip();renderBattlePass();renderPets();renderCampaign();renderFormation();renderHeroes();renderPortal();renderShop();renderMissions("daily");renderGuild();renderWorldBoss();renderTower();renderAwakening();renderAfk();renderLobbyIdentity();renderAudioSettings();AudioEngine.updateButton();updateMissionIndicators();startAfkTicker();startGlobalChat();processOfflineRewards();showScreen("lobby",{skipRender:true});auditButtonCoverage();
})();