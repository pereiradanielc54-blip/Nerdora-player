...
const RARITY_CONFIG={
 C:{code:"C",name:"Comum",color:"#FFFFFF",colorName:"Branco",rate:50,statMult:.90,ultMult:.85,speedMult:.98,rank:1},
 R:{code:"R",name:"Raro",color:"#007BFF",colorName:"Azul",rate:30,statMult:.97,ultMult:.95,speedMult:1.00,rank:2},
 EP:{code:"EP",name:"Épico",color:"#8A2BE2",colorName:"Roxo",rate:14,statMult:1.04,ultMult:1.08,speedMult:1.02,rank:3},
 SR:{code:"SR",name:"Lendário",color:"#FFA500",colorName:"Laranja",rate:5,statMult:1.12,ultMult:1.25,speedMult:1.04,rank:4},
 SSR:{code:"SSR",name:"Mítico",color:"#FF0000",colorName:"Vermelho",rate:1,statMult:1.25,ultMult:1.55,speedMult:1.06,rank:5},
 BOSS:{code:"BOSS",name:"Chefe",color:"#D75CFF",colorName:"Violeta",rate:0,statMult:1,ultMult:1,speedMult:1,rank:6}
};
const RARITY_ORDER=["C","R","EP","SR","SSR"];
const HERO_ATLAS_SRC="./assets/heroes/hero_atlas_20.webp";
const HERO_ATLAS_COLS=5;
const HERO_ATLAS_ROWS=4;
function buildHero(h){
 const explicitStats=[h.pa,h.ma,h.pd,h.md].every(Number.isFinite);
 const baseAtk=Number.isFinite(h.atk)?h.atk:Math.max(Number(h.pa)||0,Number(h.ma)||0);
 const baseDef=Number.isFinite(h.def)?h.def:Math.max(Number(h.pd)||0,Number(h.md)||0);
 const magic=h.type==="magic",secondary=Math.round(baseAtk*.28);
 return{
  ...h,
  role:h.class,
  line:h.position,
  pa:explicitStats?h.pa:(magic?secondary:baseAtk),
  ma:explicitStats?h.ma:(magic?baseAtk:secondary),
  pd:explicitStats?h.pd:Math.round(baseDef*(h.physDefMult||1)),
  md:explicitStats?h.md:Math.round(baseDef*(h.magicDefMult||.94)),
  sp:Number.isFinite(h.sp)?h.sp:h.speed,
  cr:h.cr??.08,
  dg:h.dg??.05
 };
}
const HEROES_BASE=[
 // C — COMUNS
 {id:"guarda",name:"Guarda da Fronteira",rarity:"C",color:"#FFFFFF",colorName:"Branco",img:"",atlasCol:0,atlasRow:0,emoji:"🛡️",class:"Guerreiro",position:"front",style:"melee",type:"physical",hp:1320,atk:150,def:150,speed:78,cr:.06,dg:.03,ult:{name:"Postura da Fronteira",kind:"aoe",ratio:.58,effect:"shield"}},
 {id:"acolito",name:"Acólito Aprendiz",rarity:"C",color:"#FFFFFF",colorName:"Branco",img:"",atlasCol:1,atlasRow:0,emoji:"📖",class:"Suporte",position:"back",style:"ranged",type:"magic",hp:900,atk:145,def:85,speed:96,cr:.04,dg:.06,ult:{name:"Prece Restauradora",kind:"heal",ratio:1.02,effect:"shield"}},
 {id:"batedor",name:"Batedor da Floresta",rarity:"C",color:"#FFFFFF",colorName:"Branco",img:"",atlasCol:2,atlasRow:0,emoji:"🏹",class:"Atirador",position:"back",style:"ranged",type:"physical",hp:930,atk:170,def:82,speed:110,cr:.10,dg:.10,ult:{name:"Flecha do Bosque",kind:"single",ratio:1.42,effect:"execute"}},

 // R — RAROS
 {id:"terra",name:"Guardião da Terra",rarity:"R",color:"#007BFF",colorName:"Azul",img:"",atlasCol:3,atlasRow:0,emoji:"🪨",class:"Tanque",position:"front",style:"melee",type:"physical",hp:2000,atk:200,def:220,speed:76,cr:.06,dg:.03,ult:{name:"Fortaleza Viva",kind:"aoe",ratio:.66,effect:"shield"}},
 {id:"espinho",name:"Espinho Rubro",rarity:"R",color:"#007BFF",colorName:"Azul",img:"",atlasCol:4,atlasRow:0,emoji:"🌹",class:"Guerreiro",position:"front",style:"melee",type:"physical",hp:1600,atk:250,def:150,speed:98,cr:.13,dg:.07,ult:{name:"Jardim Carmesim",kind:"aoe",ratio:.82,effect:"bleed"}},
 {id:"umbra",name:"Caçador Umbra",rarity:"R",color:"#007BFF",colorName:"Azul",img:"",atlasCol:0,atlasRow:1,emoji:"🌑",class:"Assassino",position:"back",style:"ranged",type:"physical",hp:1200,atk:280,def:110,speed:126,cr:.20,dg:.15,ult:{name:"Execução do Eclipse",kind:"single",ratio:2.05,effect:"execute"}},
 {id:"anao",name:"Canhoneiro Anão",rarity:"R",color:"#007BFF",colorName:"Azul",img:"",atlasCol:1,atlasRow:1,emoji:"💣",class:"Atirador",position:"back",style:"ranged",type:"physical",hp:1280,atk:270,def:120,speed:92,cr:.16,dg:.06,ult:{name:"Bombardeio Real",kind:"aoe",ratio:.90,effect:"pierce"}},
 {id:"aguas",name:"Irmã das Águas",rarity:"R",color:"#007BFF",colorName:"Azul",img:"",atlasCol:2,atlasRow:1,emoji:"💧",class:"Suporte",position:"back",style:"ranged",type:"magic",hp:1200,atk:245,def:120,speed:105,cr:.08,dg:.09,ult:{name:"Maré Serena",kind:"heal",ratio:1.28,effect:"shield"}},

 // EP — ÉPICOS
 {id:"quebra",name:"Quebra-Muros",rarity:"EP",color:"#8A2BE2",colorName:"Roxo",img:"",atlasCol:3,atlasRow:1,emoji:"🔨",class:"Tanque",position:"front",style:"melee",type:"physical",hp:2500,atk:275,def:285,speed:78,cr:.08,dg:.03,ult:{name:"Demolição Total",kind:"aoe",ratio:.78,effect:"break"}},
 {id:"relampago",name:"Lâmina Relâmpago",rarity:"EP",color:"#8A2BE2",colorName:"Roxo",img:"",atlasCol:4,atlasRow:1,emoji:"⚡",class:"Assassina",position:"front",style:"melee",type:"physical",hp:1450,atk:340,def:135,speed:140,cr:.23,dg:.16,ult:{name:"Mil Cortes",kind:"multi",ratio:.67,hits:5}},
 {id:"fada",name:"Fada das Lâminas",rarity:"EP",color:"#8A2BE2",colorName:"Roxo",img:"",atlasCol:0,atlasRow:2,emoji:"🧚",class:"Híbrida",position:"back",style:"ranged",type:"physical",hp:1350,atk:310,def:125,speed:132,cr:.19,dg:.18,ult:{name:"Tempestade Feérica",kind:"aoe",ratio:.80,effect:"haste"}},
 {id:"valquiria",name:"Valquíria de Gelo",rarity:"EP",color:"#8A2BE2",colorName:"Roxo",img:"",atlasCol:1,atlasRow:2,emoji:"❄️",class:"Guerreira",position:"front",style:"melee",type:"physical",hp:1850,atk:320,def:180,speed:110,cr:.14,dg:.09,ult:{name:"Inverno Eterno",kind:"aoeMagic",ratio:.88,effect:"freeze"}},
 {id:"barbanegra",name:"Barba-Negra, o Corsário",rarity:"EP",color:"#8A2BE2",colorName:"Roxo",img:"",atlasCol:2,atlasRow:2,emoji:"🏴‍☠️",class:"Atirador",position:"back",style:"ranged",type:"physical",hp:1650,atk:335,def:155,speed:102,cr:.18,dg:.08,ult:{name:"Salva do Abismo",kind:"aoe",ratio:.96,effect:"pierce"}},

 // SR — LENDÁRIOS
 {id:"crepusculo",name:"Senhor do Crepúsculo",rarity:"SR",color:"#FFA500",colorName:"Laranja",img:"",atlasCol:3,atlasRow:2,emoji:"🌘",class:"Mago",position:"back",style:"ranged",type:"magic",hp:1700,atk:420,def:180,speed:112,cr:.18,dg:.09,ult:{name:"Noite Sem Fim",kind:"aoeMagic",ratio:1.04,effect:"slow"}},
 {id:"seraphina",name:"Seraphina, a Suporte Divina",rarity:"SR",color:"#FFA500",colorName:"Laranja",img:"",atlasCol:4,atlasRow:2,emoji:"👼",class:"Suporte",position:"back",style:"ranged",type:"magic",hp:1800,atk:390,def:200,speed:114,cr:.10,dg:.10,ult:{name:"Aurora Celestial",kind:"heal",ratio:1.72,effect:"shield"}},
 {id:"ignis",name:"Ignis, o Piromante Louco",rarity:"SR",color:"#FFA500",colorName:"Laranja",img:"",atlasCol:0,atlasRow:3,emoji:"🔥",class:"Mago",position:"back",style:"ranged",type:"magic",hp:1650,atk:440,def:175,speed:118,cr:.20,dg:.08,ult:{name:"Supernova Insana",kind:"aoeMagic",ratio:1.12,effect:"bleed"}},
 {id:"aegis",name:"Aegis, o Escudo de Aço",rarity:"SR",color:"#FFA500",colorName:"Laranja",img:"",atlasCol:1,atlasRow:3,emoji:"🛡️",class:"Tanque",position:"front",style:"melee",type:"physical",hp:3100,atk:325,def:360,speed:84,cr:.09,dg:.04,ult:{name:"Bastião Absoluto",kind:"aoe",ratio:.82,effect:"shield"}},
 {id:"lyra",name:"Lyra, a Atiradora Espectral",rarity:"SR",color:"#FFA500",colorName:"Laranja",img:"",atlasCol:2,atlasRow:3,emoji:"🏹",class:"Atiradora",position:"back",style:"ranged",type:"physical",hp:1800,atk:430,def:190,speed:136,cr:.24,dg:.15,ult:{name:"Rajada Espectral",kind:"multi",ratio:.82,hits:5,effect:"haste"}},

 // SSR — MÍTICOS
 {id:"drako",name:"Drako, o Rei Dracônico",rarity:"SSR",color:"#FF0000",colorName:"Vermelho",img:"",atlasCol:3,atlasRow:3,emoji:"🐉",class:"Guerreiro Mítico",position:"front",style:"melee",type:"physical",hp:3800,atk:560,def:420,speed:128,cr:.24,dg:.10,ult:{name:"Imperador das Chamas Dracônicas",kind:"multi",ratio:1.02,hits:6,effect:"break"}},
 {id:"morgana",name:"Morgana, a Imperatriz do Vazio",rarity:"SSR",color:"#FF0000",colorName:"Vermelho",img:"",atlasCol:4,atlasRow:3,emoji:"🕳️",class:"Maga Mítica",position:"back",style:"ranged",type:"magic",hp:2900,atk:610,def:350,speed:148,cr:.28,dg:.16,ult:{name:"Horizonte do Vazio",kind:"aoeMagic",ratio:1.58,effect:"slow"}}
].map(buildHero);
const HEROES_EXTERNAL=Object.values(window.NERDORA_HERO_SOURCE||{}).sort((a,b)=>(a.number||0)-(b.number||0)).map(buildHero);
const HEROES=[...HEROES_BASE,...HEROES_EXTERNAL];

function rarityMeta(heroOrCode){
 if(typeof heroOrCode==="string")return RARITY_CONFIG[heroOrCode]||RARITY_CONFIG.BOSS;
 if(!heroOrCode?.rarity)return RARITY_CONFIG.BOSS;
 return RARITY_CONFIG[heroOrCode.rarity]||RARITY_CONFIG.BOSS;
}
...
function heroPortrait(h,cls="heroPortrait",locked=false){
 ...
 if(src&&src!=="url_placeholder.png"){
  return '<span class="'+cls+' heroPortraitImage" style="'+heroRarityStyle(h)+'"><img src="'+src+'" alt="'+h.name+'" loading="lazy" onerror="this.parentElement.classList.add(\'imgError\')"><i>'+fallback+'</i></span>';
 }
 if(Number.isInteger(h?.atlasCol)&&Number.isInteger(h?.atlasRow)){
  const x=HERO_ATLAS_COLS>1?(h.atlasCol/(HERO_ATLAS_COLS-1))*100:0;
  const y=HERO_ATLAS_ROWS>1?(h.atlasRow/(HERO_ATLAS_ROWS-1))*100:0;
  const atlasSrc=h.atlasPage===2?(window.NERDORA_HERO_ATLAS_2||HERO_ATLAS_SRC):HERO_ATLAS_SRC;
  return '<span class="'+cls+' heroPortraitAtlas" style="'+heroRarityStyle(h)+'--atlas-x:'+x+'%;--atlas-y:'+y+'%;background-image:url(\''+atlasSrc+'\')" aria-label="'+h.name+'"></span>';
 }
 return '<span class="'+cls+' heroPortraitFallback" style="'+heroRarityStyle(h)+'">'+fallback+'</span>';
}

function sortedHeroes(list=HEROES){
 return [...list].sort((a,b)=>{
  const d=rarityMeta(b).rank-rarityMeta(a).rank;if(d)return d;
  const ownDiff=Number(!!save?.owned?.[b.id])-Number(!!save?.owned?.[a.id]);if(ownDiff)return ownDiff;
...
const HERO_AFFINITY={
 ...
 zelador_catacumbas:{faction:"eclipse",factionName:"Eclipse",element:"Trevas",icon:"🏮"},
 bardo_taverna:{faction:"aurora",factionName:"Aurora",element:"Som",icon:"🎵"},
 cacador_insetos:{faction:"aurora",factionName:"Aurora",element:"Natureza",icon:"🪲"},
 alquimista_louco:{faction:"eclipse",factionName:"Eclipse",element:"Veneno",icon:"🧪"},
 gladiador_areias:{faction:"aurora",factionName:"Aurora",element:"Terra",icon:"⚔️"},
 engenhoqueiro_goblin:{faction:"eclipse",factionName:"Eclipse",element:"Metal",icon:"⚙️"},
 monge_punho_ferro:{faction:"aurora",factionName:"Aurora",element:"Luz",icon:"🥋"},
 sereia_recifes:{faction:"aurora",factionName:"Aurora",element:"Água",icon:"🌊"},
 cavaleiro_peste:{faction:"eclipse",factionName:"Eclipse",element:"Veneno",icon:"☣️"},
 samurai_espectral:{faction:"eclipse",factionName:"Eclipse",element:"Espírito",icon:"👹"},
 domador_feras:{faction:"aurora",factionName:"Aurora",element:"Natureza",icon:"🐆"},
 maquinista_juizo_final:{faction:"eclipse",factionName:"Eclipse",element:"Plasma",icon:"🔆"},
 valkyria_trovao:{faction:"aurora",factionName:"Aurora",element:"Raio",icon:"🌩️"},
 senhor_pesadelos:{faction:"eclipse",factionName:"Eclipse",element:"Pesadelo",icon:"🌙"},
 sacerdotisa_lua_sangrenta:{faction:"eclipse",factionName:"Eclipse",element:"Lua",icon:"🌕"},
 mestre_marionetes:{faction:"eclipse",factionName:"Eclipse",element:"Sombra",icon:"🎭"},
 guardiao_templo_ancestral:{faction:"aurora",factionName:"Aurora",element:"Terra",icon:"🗿"},
 atiradora_cristal_kinesis:{faction:"aurora",factionName:"Aurora",element:"Cristal",icon:"💎"},
 tita_nucleo_magmatico:{faction:"eclipse",factionName:"Eclipse",element:"Magma",icon:"🌋"},
 rainha_espelho_dimensional:{faction:"eclipse",factionName:"Eclipse",element:"Dimensão",icon:"🪞"}
};
...
const GENERIC_AWAKEN_EFFECTS=["Ultimate +8% potência","Ultimate +16% acumulado","Ultimate +24% acumulado","Ultimate +32% acumulado","Ultimate +40% acumulado"];
...
function heroStats(h){
 ...
 const statScale=h.finalBase?1:rm.statMult,speedScale=h.finalBase?1:rm.speedMult;
 const hp=Math.round((Math.round(h.hp*statScale*(1+.08*(lv-1))*sm)+b.hp)*(1+g.hp));
 const pa=Math.round((Math.round(h.pa*statScale*(1+.07*(lv-1))*sm)+b.pa)*(1+g.atk));
 const ma=Math.round((Math.round(h.ma*statScale*(1+.07*(lv-1))*sm)+b.ma)*(1+g.atk));
 const pd=Math.round((Math.round(h.pd*statScale*(1+.045*(lv-1))*sm)+b.pd)*(1+g.def));
 const md=Math.round((Math.round(h.md*statScale*(1+.045*(lv-1))*sm)+b.md)*(1+g.def));
 return{level:lv,stars,rarity:h.rarity,hp,pa,ma,pd,md,sp:Math.round(h.sp*speedScale*(1+.004*(lv-1)))+b.sp,cr:h.cr,dg:h.dg};
}
...
const SCREEN_ROUTES={
 ...
 heroDetalhe:{id:"heroDetailScreen",nav:"heroes",before:()=>renderHeroDetail("overview")},
 equipamentos:{id:"heroDetailScreen",nav:"equipment",before:()=>renderHeroDetail("equipment")},
 ...
 audio:{id:"audioSettingsScreen",nav:"",before:()=>renderAudioSettings()}
};
function routeFromScreenId(id){
 return Object.values(SCREEN_ROUTES).find(r=>r.id===id)||null;
}
function safeUiCall(fn,label="UI"){
 try{return fn()}
 catch(err){
  console.error("[Nerdora "+label+"]",err);
  const n=$("#homeUpdateNotice");
  if(n){n.textContent="⚠️ Ocorreu um erro numa tela, mas a navegação foi preservada.";n.classList.add("show")}
  return null;
 }
}
function showScreen(nameOrId,options={}){
 ...
 safeUiCall(()=>updateMissionIndicators(),"mission-indicators");
 if(typeof AudioEngine!=="undefined")safeUiCall(()=>AudioEngine.scene(id),"audio");

 if(id==="heroDetailScreen"&&options.focus==="equipment")requestAnimationFrame(()=>$("#heroEquipmentSection")?.scrollIntoView({behavior:"smooth",block:"start"}));
 return true;
}
...
function backToHeroes(){showScreen("herois")}
...
function renderResources(){
 ...
 const lv=$("#lobbySquad");if(lv)renderLobbyIdentity();
}
function renderLobbyIdentity(){
 const squad=$("#lobbySquad");if(squad){
  squad.innerHTML=SLOTS.map(slot=>{
   const id=formation?.[slot.id],h=id?hero(id):null;
 ...
 const spotlight=$("#lobbySpotlight");if(spotlight){
  ...
  spotlight.innerHTML=heroes.map(h=>'<div class="lobbySpotHero '+heroRarityClass(h)+'" style="'+heroRarityStyle(h)+'">'+heroPortrait(h,"lobbySpotPortrait")+'<span><b>'+h.name+'</b><small>'+h.rarity+' • '+h.role+'</small></span></div>').join("");
 }
}
...
function renderAwakening(){
 ...
 const effects=AWAKEN_EFFECTS[h.id]||GENERIC_AWAKEN_EFFECTS;
...
function awakenHero(id){
 ...
 save.gold-=c.gold;save.awakeningStones-=c.stones;save.awakening[id]=aw+1;persist();renderAwakening();refreshHeroManagement(id);
...
function renderAudioSettings(){
 const badge=$("#audioStateBadge"),state=$("#audioMasterState"),toggle=$("#audioMasterToggle");
 const muted=AudioEngine.muted;
 if(badge){badge.textContent=muted?"MUDO":"ATIVO";badge.classList.toggle("muted",muted)}
 if(state)state.textContent=muted?"Desligado":"Ligado";
 if(toggle)toggle.classList.toggle("muted",muted);
 AudioEngine.updateButton();
}
function toggleAudioFromSettings(){AudioEngine.toggle();renderAudioSettings()}
function testAudioSfx(){
 if(!AudioEngine.unlocked)AudioEngine.unlock();
 if(AudioEngine.muted){toast("🔇 Ative o áudio para testar");return}
 AudioEngine.sfx("reward");haptic([8,18,8]);toast("✨ Efeito de áudio reproduzido");
}
...
function playCutscene(lines,key,onDone){
 ...
 storyState={lines,index:0,key,onDone,typing:false,timer:null,full:""};
 modal.classList.add("show");
 const hint=$("#storyHint");if(hint)hint.textContent="Toque em qualquer área para continuar";
 showStoryLine();
}
...
function finishStory(){
 ...
 if(st.key){save.storySeen[st.key]=true;persist()}
 $("#storyModal")?.classList.remove("show");
 document.body.classList.remove("story-open");
 storyState=null;
 const cb=st.onDone;
 setTimeout(()=>safeUiCall(()=>cb&&cb(),"story-onDone"),120);
}
...
function starUp(heroId){
 ...
 save.shards[heroId]-=cost;save.stars[heroId]=cur+1;persist();refreshHeroManagement(heroId);
...
let worldBossStarting=false;
function beginWorldBossBattle(){
 if(worldBossStarting)return;
 worldBossStarting=true;
 try{
  document.body.classList.remove("story-open");
  $("#storyModal")?.classList.remove("show");
  save.worldBossAttempts=Math.max(0,(save.worldBossAttempts||0)-1);persist();
  const boss=new Unit(BOSS_HERO,"enemy",SLOTS[0],{level:50,scale:1});
  boss.maxHp=WORLD_BOSS_MAX_HP;boss.hp=Math.max(1,save.worldBossRemainingHp||WORLD_BOSS_MAX_HP);boss.id="enemy_vorak_F1";
  lastBattleMode="boss";landscape();battle=new Battle(playerTeam(),[boss],{mode:"boss",code:"Raid",name:BOSS_HERO.name,damage:0});
  showScreen("battleScreen",{skipRender:true});
  const title=$("#battleStageName"),log=$("#battleLog");
  if(title)title.textContent="RAID • VORAK";
  if(log)log.innerHTML="";
  battle.log("🐉 Vorak desperta com "+boss.hp.toLocaleString("pt-BR")+" HP restante.");
  renderBattle();
  Promise.resolve(battle.loop()).catch(err=>{
   console.error("[WorldBoss Battle]",err);
   toast("⚠️ A Raid encontrou um erro. Tente novamente.");
   battle=null;
   showScreen("chefe");
  }).finally(()=>{worldBossStarting=false});
 }catch(err){
  worldBossStarting=false;
  console.error("[WorldBoss Start]",err);
  toast("⚠️ Não foi possível iniciar a Raid.");
  showScreen("chefe");
 }
}
function startWorldBossBattle(){
 ...
 worldBossCutscene(()=>safeUiCall(beginWorldBossBattle,"worldboss-start"));
}
...
function renderCampaign(){
 ...
 STAGES.forEach(s=>{
  ...
  b.className="stageCard"+(!unlocked?" locked":"")+(sel?" selected":"");b.disabled=!unlocked;b.dataset.stage=s.id;
...
function renderFormation(){
 ...
 sortedHeroes(HEROES.filter(h=>save.owned[h.id])).forEach(h=>{
  const st=heroStats(h),a=affinityOf(h.id),e=document.createElement("button");e.className="rosterCard "+heroRarityClass(h)+(used().includes(h.id)?" used":"");e.dataset.hero=h.id;applyHeroRarityStyle(e,h);
 ...
 $("#selectedCount").textContent=used().length;$("#startBattle").disabled=used().length!==5;renderSynergyPanel();renderLobbyIdentity();
}
...
function firstOwnedHeroId(){
 return HEROES.find(h=>save.owned[h.id])?.id||HEROES[0].id;
}
function renderHeroes(){
 const grid=$("#heroManageGrid");if(!grid)return;
 grid.innerHTML="";
 ...
 list.forEach(h=>{
  try{
   ...
   e.type="button";
   e.className="heroManageCard heroCollectionCard "+heroRarityClass(h)+(selectedHero===h.id?" selected":"")+(!own?" locked":"");
   e.dataset.hero=h.id;
   applyHeroRarityStyle(e,h);
   e.innerHTML=`<div class="heroManageTop">${heroPortrait(h,"heroManageEmoji")}<div><div class="heroManageName">${h.name}</div><div class="heroManageMeta"><span class="rarityTag ${heroRarityClass(h)}">${h.rarity}</span> ${h.role} • ${own?"Nv."+st.level:"Não obtido"} • ${affinityOf(h.id).element}</div></div></div><div class="heroManageBars"><span>HP ${own?st.hp:"—"}</span><span>ATQ ${own?Math.max(st.pa,st.ma):"—"}</span></div><div class="heroOwnership">${own?starString(st.stars)+" • 🧩 "+(save.shards[h.id]||0):"🔒 Abrir ficha / Portal"}</div>`;
   grid.appendChild(e);
  }catch(err){
   console.error("[HeroCard]",h?.id,err);
   const e=document.createElement("button");
   e.type="button";e.className="heroManageCard heroCollectionCard";e.dataset.hero=h?.id||"";
   e.innerHTML=`<div class="heroManageName">${h?.name||"Herói"}</div><div class="heroManageMeta">Ficha disponível • toque para abrir</div>`;
   grid.appendChild(e);
  }
 });
 const legend=$(".officialRosterLegend b");
 if(legend)legend.textContent=list.length+" HERÓIS OFICIAIS";
 safeUiCall(()=>renderResources(),"heroes-resources");
}
function openHeroDetail(id,focus="overview"){
 const h=hero(id);if(!h)return;
 selectedHero=h.id;
 renderHeroDetail(focus);
 showScreen(focus==="equipment"?"equipamentos":"heroDetalhe",{skipRender:true,nav:focus==="equipment"?"equipment":"heroes",focus});
}
function renderHeroDetail(focus="overview"){
 const root=$("#heroDetail");if(!root)return;
 if(!hero(selectedHero))selectedHero=firstOwnedHeroId();
 if(focus==="equipment"&&!save.owned[selectedHero])selectedHero=firstOwnedHeroId();
 const h=hero(selectedHero),own=!!save.owned[h.id],st=heroStats(h),rar=rarityMeta(h),a=affinityOf(h.id);
 root.className="heroDetail heroDetailDedicated card "+heroRarityClass(h);
 applyHeroRarityStyle(root,h);
 if($("#heroDetailScreen"))$("#heroDetailScreen").style.setProperty("--rarity",h.color||rar.color);
 if(!own){
  root.innerHTML=`<div class="heroProfileHero lockedHeroProfile">
   <div class="heroArtStage ${heroRarityClass(h)}" style="${heroRarityStyle(h)}">${heroPortrait(h,"heroProfileArt")}<span class="heroProfileLock">🔒</span></div>
   <div class="heroProfileIdentity"><span class="rarityTag ${heroRarityClass(h)}">${h.rarity} • ${rar.name}</span><h1>${h.name}</h1><p>${h.role} • ${h.line==="front"?"Front-line":"Back-line"} • ${a.icon} ${a.element}</p><div class="heroAffinityLine"><span class="factionTag ${a.faction}">${a.icon} ${a.factionName}</span></div></div>
  </div>
  <div class="heroLockedMessage"><b>Herói ainda não obtido</b><p>Invoque este herói no Portal. Duplicatas futuras serão convertidas em Fragmentos para evolução.</p><button class="btn portalBtn" id="heroToPortal">🔮 Ir ao Portal</button></div>`;
  bindClick("heroToPortal",()=>showScreen("portal"));return;
 }
 const eq=save.equipment[h.id]||blankEquipment();
 const equipHtml=Object.keys(SLOT_NAMES).map(slot=>{
  const gid=eq[slot],g=gid?GEAR[gid]:null;
  const icon=slot==="weapon"?"⚔️":slot==="armor"?"🥋":slot==="helmet"?"🪖":"💍";
  return `<article class="heroEquipSlot ${g?"equipped":""}">
   <div class="heroEquipIcon">${g?g.icon:icon}</div>
   <div><span>${SLOT_NAMES[slot]}</span><b class="${g?rarityClass(g.rarity):""}">${g?g.name:"Vazio"}</b><small>${g?bonusText(g):"Nenhum bônus equipado"}</small></div>
   ${g?`<button class="btn ghost unequipBtn" data-slot="${slot}">Remover</button>`:""}
  </article>`;
 }).join("");
 const inv=Object.entries(save.gearInventory).filter(([,q])=>q>0).map(([gid,q])=>({g:GEAR[gid],q})).filter(x=>x.g);
 const invHtml=inv.length?inv.map(({g,q})=>`<div class="gearRow"><div class="gearIcon">${g.icon}</div><div><b class="${rarityClass(g.rarity)}">${g.name} ×${q}</b><small>${SLOT_NAMES[g.slot]} • ${bonusText(g)}</small></div><button class="btn equipBtn" data-gear="${g.id}">Equipar</button></div>`).join(""):'<div class="small emptyInventory">Nenhum equipamento no inventário. Vença fases ou visite a Loja.</div>';
 const levelCostNow=levelCost(h.id),maxLevel=st.level>=20,stars=st.stars,starCost=stars<5?STAR_COST[stars]:0,shards=save.shards[h.id]||0,starPct=stars>=5?100:Math.min(100,shards/starCost*100);
 const aw=save.awakening[h.id]||0,awCost=awakeningCost(h.id),awEligible=awakeningEligible(h.id),awMax=aw>=AWAKEN_MAX;
 const primaryAtk=Math.max(st.pa,st.ma),primaryDef=Math.max(st.pd,st.md);
 root.innerHTML=`
 <div class="heroProfileHero">
  <div class="heroArtStage ${heroRarityClass(h)}" style="${heroRarityStyle(h)}">${heroPortrait(h,"heroProfileArt")}<span class="heroProfileRarity">${h.rarity}</span></div>
  <div class="heroProfileIdentity">
   <span class="rarityTag ${heroRarityClass(h)}">${h.rarity} • ${rar.name}</span>
   <h1>${h.name}</h1>
   <p>${h.role} • ${h.line==="front"?"Front-line":"Back-line"} • ${h.type==="magic"?"Mágico":"Físico"}</p>
   <div class="heroAffinityLine"><span class="factionTag ${a.faction}">${a.icon} ${a.factionName}</span><span class="factionTag">${a.element}</span></div>
   <div class="heroProfileProgress"><span>⭐ ${starString(stars)}</span><span>🌟 Despertar ${aw}/${AWAKEN_MAX}</span><span>🧩 ${shards}</span></div>
  </div>
 </div>
 <section class="heroPrimaryStats">
  <div><span>HP</span><b>${st.hp.toLocaleString("pt-BR")}</b></div>
  <div><span>ATQ</span><b>${primaryAtk.toLocaleString("pt-BR")}</b></div>
  <div><span>DEF</span><b>${primaryDef.toLocaleString("pt-BR")}</b></div>
  <div><span>SPEED</span><b>${st.sp}</b></div>
 </section>
 <section class="heroDetailBreakdown">
  <span>ATQ Físico <b>${st.pa}</b></span><span>ATQ Mágico <b>${st.ma}</b></span><span>DEF Física <b>${st.pd}</b></span><span>DEF Mágica <b>${st.md}</b></span>
 </section>
 <section class="heroUpgradeGrid">
  <article class="heroUpgradeCard">
   <div class="heroUpgradeTitle"><span>⬆️ Nível</span><b>Nv.${st.level}/20</b></div>
   <p>Fortalece HP, Ataque, Defesa e Speed.</p>
   <button id="levelUpBtn" class="btn primary" ${maxLevel||save.gold<levelCostNow?"disabled":""}>${maxLevel?"Nível máximo":"Subir de Nível • 🪙 "+levelCostNow}</button>
   <button id="usePotionBtn" class="btn ghost" ${maxLevel||save.potions<1?"disabled":""}>🧪 Poção de Treino (${save.potions})</button>
  </article>
  <article class="heroUpgradeCard">
   <div class="heroUpgradeTitle"><span>⭐ Estrelas</span><b>${stars}/5</b></div>
   <div class="starProgress"><i style="width:${starPct}%"></i></div>
   <p>Fragmentos: <b>${shards}</b>${stars<5?" / "+starCost:""} • multiplicador ×${STAR_MULT[stars].toFixed(2)}</p>
   <button id="starUpBtn" class="btn gold" ${stars>=5||shards<starCost?"disabled":""}>${stars>=5?"5★ MÁXIMO":"Subir para "+(stars+1)+"★ • 🧩 "+starCost}</button>
  </article>
  <article class="heroUpgradeCard awakenDirectCard">
   <div class="heroUpgradeTitle"><span>🌟 Despertar</span><b>${aw}/${AWAKEN_MAX}</b></div>
   <p>${awEligible?"Requisito cumprido.":"Requer Nv.20 OU 5★."} ${awMax?"Despertar máximo alcançado.":""}</p>
   <button id="awakenDirectBtn" class="btn awakenBtn" ${!awEligible||awMax||save.gold<awCost.gold||save.awakeningStones<awCost.stones?"disabled":""}>${awMax?"DESPERTAR MÁXIMO":"Despertar • 🪙 "+awCost.gold+" + 🌟 "+awCost.stones}</button>
   <button id="heroAwakenShortcut" class="btn ghost">Ver Árvore de Talentos</button>
  </article>
  <article class="heroUpgradeCard ultimateCard">
   <div class="heroUpgradeTitle"><span>💥 Ultimate</span><b>${h.ult.name}</b></div>
   <p>Potência base ×${(h.ult.ratio*(h.ultimateFinal?1:rar.ultMult)).toFixed(2)} • escala com raridade e Despertar.</p>
  </article>
 </section>
 <section id="heroEquipmentSection" class="heroEquipmentSection">
  <div class="heroSectionHeading"><div><span class="small">EQUIPAMENTO INDIVIDUAL</span><h2>Arma, Armadura, Elmo e Acessório</h2></div><button id="heroOpenShop" class="btn ghost">🛒 Loja</button></div>
  <div class="heroEquipmentGrid">${equipHtml}</div>
  <div class="inventoryTitle">Inventário disponível</div>
  <div class="gearInventory">${invHtml}</div>
 </section>`;
 bindClick("levelUpBtn",()=>levelUp(h.id));
 bindClick("usePotionBtn",()=>usePotion(h.id));
 bindClick("starUpBtn",()=>starUp(h.id));
 bindClick("awakenDirectBtn",()=>awakenHero(h.id));
 bindClick("heroAwakenShortcut",()=>{selectedAwakeningHero=h.id;showScreen("despertar")});
 bindClick("heroOpenShop",()=>showScreen("loja"));
 $$(".unequipBtn").forEach(b=>b.onclick=()=>unequip(h.id,b.dataset.slot));
 $$(".equipBtn").forEach(b=>b.onclick=()=>equip(h.id,b.dataset.gear));
 renderResources();
 if(focus==="equipment")requestAnimationFrame(()=>$("#heroEquipmentSection")?.scrollIntoView({behavior:"smooth",block:"start"}));
}
function refreshHeroManagement(id=selectedHero){
 if(id&&hero(id))selectedHero=id;
 renderHeroes();
 if($("#heroDetailScreen")?.classList.contains("active"))renderHeroDetail();
 renderFormation();
 renderResources();
}
function levelUp(id){
 const lv=save.heroLevels[id]||1,cost=levelCost(id);if(lv>=20||save.gold<cost)return;
 save.gold-=cost;save.heroLevels[id]=lv+1;trackEvent("levelUps",1);persist();refreshHeroManagement(id);toast("⬆ "+hero(id).name+" subiu para Nv."+(lv+1));
}
function usePotion(id){
 if(save.potions<1||(save.heroLevels[id]||1)>=20)return;
 save.potions--;save.heroLevels[id]++;trackEvent("levelUps",1);persist();refreshHeroManagement(id);toast("🧪 Nível aumentado com Poção");
}
function equip(heroId,gearId){
 ...
 save.gearInventory[gearId]--;save.equipment[heroId][slot]=gearId;persist();refreshHeroManagement(heroId);toast(g.icon+" "+g.name+" equipado");
}
function unequip(heroId,slot){
 const gid=save.equipment[heroId][slot];if(!gid)return;
 addGear(gid,1);save.equipment[heroId][slot]=null;persist();refreshHeroManagement(heroId);toast("Equipamento removido");
}
...
function renderPortal(){
 ...
 const art=$("#limitedHeroPortrait");if(art){const h=hero(LIMITED_HERO_ID);art.innerHTML=heroPortrait(h,"limitedHeroPortraitArt");art.style.setProperty("--rarity",h.color)}
}
...
function summonRarityFlash(results){
 ...
 flash.innerHTML='<div class="rarityFlashCore">'+heroPortrait(elite,"rarityFlashPortrait")+'<span>'+meta.code+'</span><b>'+meta.name.toUpperCase()+'</b><small>'+elite.name+'</small></div>';
...
class Battle{
 ...
 async ult(u){
  ...
  const z=u.h.ult,aw=u.awakening||0,ratio=z.ratio*(u.h.ultimateFinal?1:rarityMeta(u.h).ultMult)*(1+aw*.08);
  ...
  if(z.kind==="heal"){
   ...
   for(let i=0;i<(z.hits||5)+extra;i++){const t=this.weak(this.enemies(u));if(!t)break;this.damage(u,t,u.type,ratio,{ult:true,noDodge:true,pierce:z.effect==="pierce"});await this.delay(220)}
  }else{
...
function renderBattle(){
 ...
 $("#turnOrder").innerHTML=battle.q.map((u,i)=>`<span class="turnChip${i<battle.i?" done":""}${battle.current===u?" current":""}" style="${heroRarityStyle(u.h)}">${heroPortrait(u.h,"turnPortrait")}<span>${u.name.split(" ")[0]}</span><b>${u.sp}</b></span>`).join(""); const aura=$("#teamAuraLabel");if(aura)aura.textContent=battle.synergy?.active?("✨ "+battle.synergy.name+" "+battle.synergy.count+"/5"):"SEM AURA";
...
function finishBattle(win,ctx){
 ...
 if(ctx.mode==="boss"){
  ...
  next.style.display="none";retry.style.display="none";$("#resultMenuBtn").textContent="Voltar ao Chefe Épico";openResultModal();return;
 }
 if(ctx.mode==="tower"){
  ...
  renderTower();openResultModal();return;
 }
 if(ctx.mode==="guildWar"){
  ...
  renderGuild();next.style.display="none";retry.style.display="none";openResultModal();return;
 }
 if(ctx.mode==="arena"){
 ...
 renderMissions("daily");openResultModal();
}
function openResultModal(){
 const modal=$("#resultModal"),card=modal?.querySelector(".modalCard");
 if(card){card.scrollTop=0;card.style.overflowY="auto"}
 if(modal){
...
const UI_ROUTE_BUTTONS={
 ...
 goAudioSettings:"audio",
 quickCampaignTab:"campanha",
 quickHeroesTab:"herois",
 quickPortalTab:"portal"
};
const BACK_TO_LOBBY_BUTTONS=[
 "vipBack","battlePassBack","petsBack","campaignBack","heroesBack","portalBack","arenaBack","shopBack",
 "missionsBack","guildBack","worldBossBack","towerBack","afkBack","awakeningBack","formationBack","audioSettingsBack"
];
...
function openNav(key){
 ...
 return showScreen(map[key]||key);
}
function bindClick(id,handler){
 const el=$("#"+id);
 if(!el){console.warn("[UI] Botão não encontrado:",id);return false}
 if(el.dataset.boundClick==="1")return true;
 el.dataset.boundClick="1";
 el.addEventListener("click",handler);
 return true;
}
function bindRouteButtons(){
 Object.entries(UI_ROUTE_BUTTONS).forEach(([id,route])=>bindClick(id,()=>{
 ...
 BACK_TO_LOBBY_BUTTONS.forEach(id=>bindClick(id,backToLobby));
 bindClick("heroDetailBack",backToHeroes);
 bindClick("heroDetailLobby",backToLobby);
...
function bindStaticControls(){
 ...
 bindClick("muteBtn",()=>{AudioEngine.toggle();renderAudioSettings()});
 bindClick("audioMasterToggle",()=>toggleAudioFromSettings());
 bindClick("audioTestSfx",()=>testAudioSfx());
 bindClick("storyNext",()=>nextStory());bindClick("storySkip",()=>skipStory());
 const storyModalEl=$("#storyModal");
 if(storyModalEl&&!storyModalEl.dataset.storyAdvanceBound){
  storyModalEl.dataset.storyAdvanceBound="1";
  storyModalEl.addEventListener("click",e=>{if(!e.target.closest("button"))nextStory()});
 }
...
function handleCentralButton(btn,e){
 ...
 if(BACK_TO_LOBBY_BUTTONS.includes(id)||id==="heroDetailLobby"||id==="battleBack"){backToLobby();return true}
 if(id==="heroDetailBack"){backToHeroes();return true}

 // Lobby / Live Ops / áudio / atualização.
 if(id==="quickBattle"){quick();return true}
 if(id==="refreshArena"){generateArenaOpponents();renderArena();return true}
 if(id==="missionsGuildBtn"){showScreen("guilda");return true}
 if(id==="startWorldBoss"){startWorldBossBattle();return true}
 if(id==="startTower"){startTowerBattle();return true}
 if(id==="claimAfkBtn"){claimAfk(false);return true}
 if(id==="unlockPremiumPass"){unlockPremiumPass();return true}
 if(id==="chatToggle"){toggleGlobalChat();return true}
 if(id==="muteBtn"||id==="audioMasterToggle"){toggleAudioFromSettings();return true}
 if(id==="audioTestSfx"){testAudioSfx();return true}
 if(id==="installAppBtn"||id==="splashInstallBtn"){install();return true}
 if(id==="checkUpdateBtn"){checkUpdate(true).then(has=>{if(has)updateNow()});return true}
 if(id==="splashUpdateBtn"){updateNow();return true}

 // História / tabs / combate / resultado.
 if(id==="storyNext"){nextStory();return true}
 if(id==="storySkip"){skipStory();return true}
 if(id==="dailyTab"){renderMissions("daily");return true}
 if(id==="achievementTab"){renderMissions("achievements");return true}
 if(id==="startBattle"){startBattle();return true}
 if(id==="summonOne"){summonFromBanner(1,"standard");return true}
 if(id==="summonTen"){summonFromBanner(10,"standard");return true}
 if(id==="eventSummonOne"){summonFromBanner(1,"event");return true}
 if(id==="eventSummonTen"){summonFromBanner(10,"event");return true}
 if(id==="speed1"){if(battle){battle.speed=1;renderBattle()}return true}
 if(id==="speed2"){if(battle){battle.speed=2;renderBattle()}return true}
 if(id==="pauseBtn"){if(battle&&!battle.ended){battle.paused=!battle.paused;renderBattle()}return true}
 if(id==="modeBtn"){if(battle&&!battle.ended){battle.mode=battle.mode==="manual"?"auto":"manual";battle.log("🤖 Ultimate: "+battle.mode.toUpperCase(),"ultimate");renderBattle()}return true}
 if(id==="nextStageBtn"){goNextStage();return true}
 if(id==="retryBtn"){retryStage();return true}
 if(id==="resultMenuBtn"){goResultMenu();return true}

 // Gestão individual do herói.
 if(id==="heroToPortal"){showScreen("portal");return true}
 if(id==="levelUpBtn"){levelUp(selectedHero);return true}
 if(id==="usePotionBtn"){usePotion(selectedHero);return true}
 if(id==="starUpBtn"){starUp(selectedHero);return true}
 if(id==="awakenDirectBtn"){awakenHero(selectedHero);return true}
 if(id==="heroAwakenShortcut"){selectedAwakeningHero=selectedHero;showScreen("despertar");return true}
 if(id==="heroOpenShop"){showScreen("loja");return true}
 if(id==="awakenHeroBtn"){awakenHero(selectedAwakeningHero);return true}

 // Mascotes.
 if(id==="feedPetBtn"){feedPet(selectedPet);return true}
 if(id==="activatePetBtn"){activatePet(selectedPet);return true}
 if(id==="unlockPetBtn"){unlockPet(selectedPet);return true}
 if(btn.classList.contains("petCard")&&btn.dataset.pet){selectedPet=btn.dataset.pet;renderPets();return true}

 // Campanha e formação.
 if(btn.classList.contains("stageCard")&&btn.dataset.stage){
  const st=STAGES.find(x=>String(x.id)===String(btn.dataset.stage));
  if(st&&st.id<=save.unlockedStage){save.selectedStage=st.id;persist();renderCampaign();renderFormation();showScreen("formacao")}
  return true;
 }
 if(btn.classList.contains("slot")&&btn.dataset.slot){activeSlot=btn.dataset.slot;renderFormation();return true}
 if(btn.classList.contains("rosterCard")&&btn.dataset.hero){
  const hid=btn.dataset.hero;
  Object.keys(formation).forEach(k=>{if(formation[k]===hid)formation[k]=null});
  formation[activeSlot]=hid;
  const i=SLOTS.findIndex(x=>x.id===activeSlot);activeSlot=SLOTS[Math.min(i+1,4)].id;
  persist();renderFormation();return true;
 }
 if(btn.classList.contains("heroCollectionCard")&&btn.dataset.hero){openHeroDetail(btn.dataset.hero);return true}
 if(btn.classList.contains("lobbySquadHero")&&btn.dataset.hero){openHeroDetail(btn.dataset.hero);return true}

 // Pass / missões / guilda / loja / arena.
 if(btn.classList.contains("claimPassBtn")){claimPassReward(btn.dataset.tier,btn.dataset.premium==="1");return true}
 if(btn.classList.contains("claimMissionBtn")){claimDaily(btn.dataset.id);return true}
 if(btn.classList.contains("claimAchievementBtn")){claimAchievement(btn.dataset.id);return true}
 if(btn.classList.contains("joinGuildBtn")){joinGuild(btn.dataset.id);return true}
 if(btn.classList.contains("guildUpgradeBtn")){upgradeGuild(btn.dataset.key);return true}
 if(btn.classList.contains("guildWarBtn")){startGuildWarBattle(btn.dataset.castle);return true}
 if(btn.classList.contains("buyBtn")){buyShop(btn.dataset.id);return true}
 if(btn.classList.contains("duelBtn")){
  const o=arenaOpponents.find(x=>x.id===btn.dataset.id);startArenaBattle(o);return true;
 }

 // Equipamentos.
 if(btn.classList.contains("unequipBtn")){unequip(selectedHero,btn.dataset.slot);return true}
 if(btn.classList.contains("equipBtn")){equip(selectedHero,btn.dataset.gear);return true}

 // Ultimate manual.
 if(btn.classList.contains("ultBtn")&&btn.dataset.uid){
  const u=battle?.p?.find(x=>x.id===btn.dataset.uid);
  if(u&&u.rage>=100&&!u.dead&&battle.mode!=="auto"){
   u.queued=!u.queued;
   battle.log((u.queued?"⏳ ":"↩️ ")+u.name+(u.queued?": Ultimate armada.":": Ultimate cancelada."),"ultimate");
   renderBattle();
  }
  return true;
 }
...
document.addEventListener("click",e=>{
 ...
 if(!btn)return;
 const handled=handleCentralButton(btn,e);
 if(handled){
  e.preventDefault();
...
function auditButtonCoverage(){
 const knownStatic=new Set([
  ...Object.keys(UI_ROUTE_BUTTONS),...BACK_TO_LOBBY_BUTTONS,
  "heroDetailBack","heroDetailLobby","battleBack","quickBattle","refreshArena","missionsGuildBtn",
  "startWorldBoss","startTower","claimAfkBtn","unlockPremiumPass","chatToggle","muteBtn",
  "audioMasterToggle","audioTestSfx","installAppBtn","splashInstallBtn","checkUpdateBtn","splashUpdateBtn",
  "storyNext","storySkip","dailyTab","achievementTab","startBattle","summonOne","summonTen",
  "eventSummonOne","eventSummonTen","speed1","speed2","pauseBtn","modeBtn","nextStageBtn",
  "retryBtn","resultMenuBtn"
 ]);
 const dynamicIds=new Set(["heroToPortal","levelUpBtn","usePotionBtn","starUpBtn","awakenDirectBtn","heroAwakenShortcut","heroOpenShop","awakenHeroBtn","feedPetBtn","activatePetBtn","unlockPetBtn"]);
 const unknown=$$("button[id]").filter(b=>!knownStatic.has(b.id)&&!dynamicIds.has(b.id)).map(b=>b.id);
 if(unknown.length)console.warn("[UI Audit] IDs sem rota central explícita:",unknown);
 return unknown;
}
...
if("serviceWorker"in navigator){addEventListener("load",()=>{navigator.serviceWorker.register("./sw-5.1.1.js",{scope:"./",updateViaCache:"none"}).then(r=>{reg=r;r.update().catch(()=>{})}).catch(()=>{});checkUpdate(false)})}else addEventListener("load",()=>checkUpdate(false));
setTimeout(()=>hideSplash(0),3500);

[
 ["resetDailyMissions",()=>resetDailyMissions()],
 ["dailyArenaReset",()=>dailyArenaReset()],
 ["dailyWorldBossReset",()=>dailyWorldBossReset()],
 ["dailyGuildWarReset",()=>dailyGuildWarReset()],
 ...
 ["portal",()=>renderPortal()],
 ["shop",()=>renderShop()],
 ["missions",()=>renderMissions("daily")],
 ["guild",()=>renderGuild()],
 ["worldBoss",()=>renderWorldBoss()],
 ["tower",()=>renderTower()],
 ["awakening",()=>renderAwakening()],
 ["afk",()=>renderAfk()],
 ["lobbyIdentity",()=>renderLobbyIdentity()],
 ["audioSettings",()=>renderAudioSettings()],
 ["audioButton",()=>AudioEngine.updateButton()],
 ["missionIndicators",()=>updateMissionIndicators()],
 ["afkTicker",()=>startAfkTicker()],
 ["globalChat",()=>startGlobalChat()],
 ["offlineRewards",()=>processOfflineRewards()]
].forEach(([label,fn])=>safeUiCall(fn,"init:"+label));
showScreen("lobby",{skipRender:true});
auditButtonCoverage();
})();