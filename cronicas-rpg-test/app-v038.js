...
function triggerAdaptiveRecovery(actor,text){
  ...
  const pool=state.campaignId==="derenfall"?currentDerenActions():currentCampaignActions();
  const attempted=normalizedActionKey(text);
  const candidates=rankActionsWithMemory(pool).filter(function(a){return normalizedActionKey(a.label)!==attempted});
...
function resolveCampaignActionRoll(actor,r,actionId){
  ...
  if(!r.success){
    ...
    if(r.outcome==="partial_failure"){
      addStory("master","Mestre Máquina",(a.fail||"A tentativa encontra resistência.")+" A margem foi pequena: surge informação parcial, uma nova posição ou uma forma mais clara de tentar novamente.",r.formula);advanceTime(6);return;
    }
    state.pressure=Math.min(5,(state.pressure||0)+1);
    addStory("master","Mestre Máquina",(a.fail||"A tentativa não fecha a questão.")+" A falha é severa e aumenta a pressão, mas não remove as outras rotas de investigação.",r.formula);advanceTime(8);return;
  }
  ...
  advanceTime(r.outcome==="critical"?4:r.outcome==="strong_success"?5:7);updateCampaignObjective();
}
const NPC_CATALOG={
  derenfall:{
...
const CAMPAIGN_FRONTS={
  derenfall:[
    {id:"fenda",name:"Propagação da Fenda",agenda:"Espalhar perda de identidade para além de Derenfall."},
    {id:"ecos",name:"Ecos dos Moradores",agenda:"Substituir rotinas reais por memórias incompletas."},
    {id:"colecionador",name:"Interesse do Colecionador",agenda:"Preservar lembranças mesmo quando isso apaga autonomia."}
  ],
  vidro:[
    {id:"casas",name:"Guerra das Casas",agenda:"Transformar a dúvida dinástica em disputa aberta."},
    {id:"arquivos",name:"Apagamento dos Arquivos",agenda:"Controlar documentos, testemunhas e cadeia de custódia."},
    {id:"coroa",name:"Crise da Coroa",agenda:"Forçar uma decisão sucessória antes que a verdade esteja completa."}
  ],
  coro:[
    {id:"contagio",name:"Contágio do Coro",agenda:"Espalhar a ressonância entre mineiros acordados."},
    {id:"producao",name:"Pressão da Produção",agenda:"Manter a mina ativa mesmo com o risco crescente."},
    {id:"rede",name:"Despertar da Rede",agenda:"Reconectar memórias humanas à montanha."}
  ]
};
function primaryFrontId(cid=state?.campaignId){
  return cid==="derenfall"?"fenda":cid==="vidro"?"casas":cid==="coro"?"contagio":null;
}
function ensureFrontState(){
  if(!state)return;
  if(!state.frontClocks)state.frontClocks={};
  (CAMPAIGN_FRONTS[state.campaignId]||[]).forEach(function(front){
    if(typeof state.frontClocks[front.id]!=="number")state.frontClocks[front.id]=0;
  });
  if(!Array.isArray(state.frontLog))state.frontLog=[];
  const primary=primaryFrontId();
  if(primary&&(state.pressure||0)>0&&Object.values(state.frontClocks).every(function(v){return !v})){
    state.frontClocks[primary]=Math.min(6,state.pressure||0);
  }
}
function advanceFront(id,amount=1,reason=""){
  if(!id||!state)return;
  ensureFrontState();
  const before=state.frontClocks[id]||0,after=Math.max(0,Math.min(6,before+amount));
  state.frontClocks[id]=after;
  if(after!==before){
    state.frontLog.unshift({id:uid(),frontId:id,before:before,after:after,reason:reason,at:Date.now()});
    state.frontLog=state.frontLog.slice(0,30);
  }
}
function frontForWorldEvent(eventId){
  const map={
    deren_beat_5:"fenda",deren_beat_10:"ecos",deren_beat_15:"colecionador",
    vidro_beat_4:"casas",vidro_beat_8:"arquivos",vidro_beat_12:"coroa",vidro_beat_16:"casas",vidro_beat_20:"coroa",
    coro_beat_4:"contagio",coro_beat_8:"rede",coro_beat_12:"producao",coro_beat_16:"contagio",coro_beat_20:"rede"
  };
  return map[eventId]||primaryFrontId();
}
function campaignFrontStatus(){
  ensureFrontState();
  return (CAMPAIGN_FRONTS[state?.campaignId]||[]).map(function(front){
    return {...front,value:Math.max(0,Math.min(6,state.frontClocks?.[front.id]||0))};
  });
}
function renderFrontClocks(){
  if(!ui.frontClockList||!state)return;
  const fronts=campaignFrontStatus();
  ui.frontClockList.innerHTML=fronts.length
    ? "<div class='fronts-head'><span>FRENTES DO MUNDO</span><small>Ameaças avançam separadas do progresso da companhia</small></div>"+
      fronts.map(function(front){
        const dots=Array.from({length:6},function(_,i){return "<i class='"+(i<front.value?"filled":"")+"'></i>"}).join("");
        return "<article class='front-clock'><div><strong>"+esc(front.name)+"</strong><small>"+esc(front.agenda)+"</small></div><div class='front-segments' aria-label='"+front.value+" de 6'>"+dots+"</div></article>";
      }).join("")
    : "";
}
function oracleHash(input){
  let h=2166136261,s=String(input||"");
  for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619)}
  return h>>>0;
}
function oracleChoose(context,options){
  const list=(options||[]).filter(Boolean);if(!list.length)return null;
  ensureStateShape();
  if(!Array.isArray(state.oracleLog))state.oracleLog=[];
  const seed=[state.campaignId,state.location,state.director?.beats||0,state.worldMinutes||0,context,list.map(function(x){return x.label||x.name||String(x)}).join("|")].join("::");
  const chosen=list[oracleHash(seed)%list.length];
  state.oracleLog.unshift({id:uid(),context:context,choice:chosen.label||chosen.name||String(chosen),optionCount:list.length,at:Date.now()});
  state.oracleLog=state.oracleLog.slice(0,24);
  return chosen;
}
function buildSessionSummary(){
  if(!state)return null;
  ensureStateShape();
  const metrics=runMetrics(),wt=worldTime();
  return {
    version:"0.3.8",
    campaign:CAMPAIGNS[state.campaignId]?.title||state.campaign,
    campaignId:state.campaignId,location:state.location,objective:state.objective,ending:state.ending||null,
    worldDay:wt.day,worldTime:wt.time,
    metrics:{decisions:metrics.decisions||0,rolls:metrics.rolls||0,travels:metrics.travels||0,estimatedFastMinutes:Number(metrics.estimatedFastMinutes.toFixed(1))},
    visitedLocations:[...(state.visitedLocations||[])],
    conclusions:(state.clues||[]).map(function(id){return CLUES[id]?.title||id}),
    evidence:(state.rawEvidence||[]).map(function(item){const id=typeof item==="string"?item:item.id;return EVIDENCE[id]?.title||id}),
    fronts:campaignFrontStatus().map(function(f){return {id:f.id,name:f.name,value:f.value,max:6}}),
    relationships:Object.values(state.npcRelations||{}).map(function(r){return {name:r.name,status:relationshipLabel(r)}}),
    oracleDecisions:(state.oracleLog||[]).length,generatedAt:new Date().toISOString()
  };
}
function formatSessionSummary(s){
  if(!s)return"";
  return [
    "CRÔNICAS DE NERDORA — RESUMO DA SESSÃO",s.campaign,
    "Local atual: "+s.location,"Objetivo: "+s.objective,s.ending?"Desfecho: "+s.ending:null,
    "Tempo no mundo: Dia "+s.worldDay+" • "+s.worldTime,
    "Decisões: "+s.metrics.decisions+" • Rolagens: "+s.metrics.rolls+" • Viagens: "+s.metrics.travels,
    "Locais visitados: "+(s.visitedLocations.join(", ")||"—"),
    "Conclusões: "+(s.conclusions.join(", ")||"—"),
    "Evidências: "+(s.evidence.join(", ")||"—"),
    "Frentes: "+(s.fronts.map(function(f){return f.name+" "+f.value+"/"+f.max}).join(" • ")||"—"),
    "Relações: "+(s.relationships.map(function(r){return r.name+" ("+r.status+")"}).join(", ")||"—")
  ].filter(Boolean).join("\n");
}
async function copySessionSummary(){
  const s=buildSessionSummary();if(!s)return;
  state.sessionSummary=s;const text=formatSessionSummary(s);
  try{
  ...
  }catch(e){console.warn("Falha ao copiar resumo",e);toast("Não foi possível copiar o resumo neste navegador.");}
  renderSessionSummary();
}
function renderSessionSummary(){
  if(!ui.sessionSummaryPreview||!state)return;
  const s=buildSessionSummary();
  ui.sessionSummaryPreview.textContent=s.visitedLocations.length+" locais • "+s.conclusions.length+" conclusões • "+s.metrics.rolls+" rolagens • "+s.fronts.filter(function(f){return f.value>0}).length+" frentes ativas";
}
...
function directorSignature(){
  ...
  return [
    ...
    JSON.stringify(state.frontClocks||{})
  ].join(";");
}
...
function directorOmenText(){
  if(state.campaignId==="derenfall")return "O silêncio deixa de parecer estático: pequenos sinais mostram que a anomalia está se preparando para avançar.";
  if(state.campaignId==="vidro")return "Rumores, mensageiros e portas fechadas indicam que as Casas estão prestes a mover uma peça importante.";
  if(state.campaignId==="coro")return "A vibração nas pedras muda de ritmo. O grupo percebe que a situação na mina está perto de piorar.";
  return "Há sinais claros de que o mundo está prestes a reagir às escolhas e ao tempo gasto.";
}
function fireDirectorWorldEvents(){
  ensureDirectorState();
  const events=DIRECTOR_WORLD_EVENTS[state.campaignId]||[],d=state.director;
  events.forEach(function(ev){
    const omenKey="omen:"+ev.id,key="director:"+ev.id;
    if((d.beats||0)===Math.max(1,ev.beat-1)&&!state.eventFired.includes(omenKey)){
      state.eventFired.push(omenKey);
      const omen=directorOmenText();
      recordWorldEvent("Presságio",omen,"omen",state.location);addStory("system","Presságio",omen);
    }
    if((d.beats||0)>=ev.beat&&!state.eventFired.includes(key)){
      ...
      advanceFront(frontForWorldEvent(ev.id),1,"evento mundial: "+ev.id);
...
function directorIntervention(actor,text){
  ...
  if(state.campaignId==="derenfall"){
    ...
    if(options.length){
      const a=oracleChoose("director_action",options)||options[0];
  ...
  const exits=connectedLocations(state.location||"");
  if(exits.length){
    const exit=oracleChoose("director_exit",exits)||exits[0];
    addStory("system","Diretor Narrativo","Enquanto vocês reconsideram a situação, algo muda no ambiente e torna "+exit+" uma alternativa mais atraente. O grupo continua livre para permanecer aqui.");
    return true;
  }
  state.pressure=Math.min(5,(state.pressure||0)+1);
...
function createInitialState(campaignId){
  ...
  return {version:"0.3.8",campaignId:campaignId,campaign:c.title,location:c.location,worldMinutes:18*60+40,day:1,pressure:0,
...
function advanceTime(n=5){
  ...
  if(state.pressure>before)advanceFront(primaryFrontId(),state.pressure-before,"tempo e pressão");
...
function classifyRollOutcome(d20,total,df){
  const margin=total-df;
  if(d20===20)return {key:"critical",label:"Sucesso crítico",success:true,margin:margin};
  if(margin>=5)return {key:"strong_success",label:"Sucesso forte",success:true,margin:margin};
  if(margin>=0)return {key:"success",label:"Sucesso",success:true,margin:margin};
  if(margin>=-4)return {key:"partial_failure",label:"Falha com progresso",success:false,margin:margin};
  return {key:"hard_failure",label:"Falha severa",success:false,margin:margin};
}
function handleRollTap(playerId){
  ...
  const outcome=classifyRollOutcome(d20,total,p.df);
  const result={id:uid(),rollId:p.id,playerId,who:actor.name,d20,bonus,total,stat:p.stat,df:p.df,success:outcome.success,critical:d20===20,fumble:d20===1,margin:outcome.margin,outcome:outcome.key,outcomeLabel:outcome.label,formula:`1d20 (${d20}) + ${p.stat} ${baseBonus>=0?"+":""}${baseBonus}${gearBonus?` + Equip. ${gearBonus}`:""} = ${total} vs DF ${p.df}`};
  ...
  addStory("system","Resultado do dado",`${actor.name}: ${result.formula} • ${outcome.label}`,result.formula);
...
function resolveDerenActionRoll(actor,r,actionId){
  ...
  if(!r.success){
    ...
    if(r.outcome==="partial_failure"){
      addStory("master","Mestre Máquina",actionFailureNarrative(a)+" A falha foi estreita: vocês obtêm uma posição melhor e entendem parte do obstáculo. A próxima abordagem começa com essa vantagem.",r.formula);
      advanceTime(4);return;
    }
    state.pressure=Math.min(5,(state.pressure||0)+1);
    addStory("master","Mestre Máquina",actionFailureNarrative(a)+" A falha é severa e o mundo reage: tempo, exposição ou perigo aumentam, mas uma rota de continuação permanece aberta.",r.formula);advanceTime(6);return;
  }
...
function resolveRollContext(actor,r,ctx){
  ...
  if(ctx.kind==="generic"){
    if(r.outcome==="critical"||r.outcome==="strong_success"){
      addStory("master","Mestre Máquina","A tentativa funciona com margem. Além de alcançar o objetivo imediato, a companhia conquista uma posição favorável para a próxima ação.",r.formula);advanceTime(r.outcome==="critical"?3:4);
    }else if(r.success){
      addStory("master","Mestre Máquina","A tentativa funciona. O resultado altera a cena a favor da companhia, respeitando o método descrito.",r.formula);advanceTime(5);
    }else if(r.outcome==="partial_failure"){
      state.pressure=Math.min(5,(state.pressure||0)+1);addStory("master","Mestre Máquina","A tentativa não alcança tudo que pretendia, mas produz progresso real: uma informação parcial, uma posição melhor ou uma nova opção aparece junto com o custo.",r.formula);advanceTime(7);
    }else{
      state.pressure=Math.min(5,(state.pressure||0)+2);addStory("master","Mestre Máquina","A tentativa falha de forma clara e a situação piora, mas não congela: o Mestre introduz uma consequência concreta e preserva pelo menos uma rota de continuação.",r.formula);advanceTime(10);
    }
    return;
  }
...
function renderCampaignThreads(){
  ...
  let threads=[];
  if(state.campaignId==="derenfall")threads=[
    ["O silêncio e o desaparecimento",hasClue("silencio")||hasClue("simultaneo")],
    ["Sino e fundações antigas",hasClue("sino")||hasClue("fundacao")||hasClue("nhal")],
    ["Identidade e memória",hasClue("memoria")||hasClue("afetos")||hasClue("lapides")],
    ["Rotas para a Fenda",["route_igreja","route_capela","route_bosque","route_poco"].some(hasFlag)],
    ["Moradores e âncoras",hasClue("moradores")||hasClue("ancoras")],
    ["O Colecionador",hasClue("colecionador")]
  ];
  else if(state.campaignId==="vidro")threads=[
  ...
  ui.campaignThreads.innerHTML="<div class='threads-head'><span>TRILHAS DE PROGRESSO</span><small>"+threads.filter(function(x){return x[1]}).length+"/"+threads.length+" avançadas</small></div><div class='thread-grid'>"+threads.map(function(t){return "<div class='thread-chip "+(t[1]?"done":"open")+"'><i>"+(t[1]?"✓":"?")+"</i><span>"+esc(t[0])+"</span></div>"}).join("")+"</div>";
}
function renderState(){
  ...
  discoverNpcsForLocation(state.location);renderCampaignThreads();renderFrontClocks();renderStory();renderClues();renderQuickActions();renderCampaignMap();renderDicePrompt();renderSheet();renderNpcRelations();renderWorldEventCards();renderSessionSummary();renderMasterMemory();checkLastRoll();
}
...
function submitIntent(text=null){
  const v=String(text||"").trim();if(!v||!state)return;
