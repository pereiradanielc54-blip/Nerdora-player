...
var deferredInstallPrompt=null,isStandalone=window.matchMedia&&window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===true,audioCtx=null,musicTimer=null,musicStep=0,reconnectTimer=null,migrationTimer=null;
...
function beep(freq,dur,vol,type){var ctx=getAudio();if(!ctx)return;try{var o=ctx.createOscillator(),g=ctx.createGain();o.type=type||"sine";o.frequency.value=freq;g.gain.value=vol||.025;o.connect(g);g.connect(ctx.destination);var t=ctx.currentTime;g.gain.setValueAtTime(g.gain.value,t);g.gain.exponentialRampToValueAtTime(.0001,t+dur);o.start(t);o.stop(t+dur)}catch(e){}}
function sfx(kind){if(!S.soundOn)return;if(kind==="mark"){beep(620,.08,.025,"sine")}else if(kind==="ball"){beep(420,.08,.025,"triangle");setTimeout(function(){beep(760,.12,.022,"sine")},80)}else if(kind==="win"){[523,659,784,1047].forEach(function(f,i){setTimeout(function(){beep(f,.28,.035,"sine")},i*110)})}else if(kind==="chat"){beep(840,.08,.018,"sine")}}
...
function ensureArcadePlayer(){
  ...
  return ensureArcadeApi().then(function(){return new Promise(function(resolve){
    ...
    arcadePlayer=new YT.Player("arcadeMusicHost",{width:"2",height:"2",videoId:ARCADE_VIDEO_ID,playerVars:{controls:0,disablekb:1,playsinline:1,rel:0,modestbranding:1,start:ARCADE_START},events:{
      onReady:function(e){arcadePlayerReady=true;try{e.target.setVolume(ARCADE_VOLUME)}catch(x){}resolve(e.target)},
...
function startArcadeMusic(looping){
  ...
  ensureArcadePlayer().then(function(p){
    try{p.setVolume(ARCADE_VOLUME);if(looping||p.getCurrentTime()<ARCADE_START||p.getCurrentTime()>=ARCADE_END)p.seekTo(ARCADE_START,true);p.playVideo();startArcadeLoopWatch()}catch(e){}
...
function ensureNyanPlayer(){
  ...
  return ensureArcadeApi().then(function(){return new Promise(function(resolve){
    ...
    nyanPlayer=new YT.Player("nyanMusicHost",{width:"2",height:"2",videoId:NYAN_VIDEO_ID,playerVars:{controls:0,disablekb:1,playsinline:1,rel:0,modestbranding:1,start:NYAN_START},events:{
      onReady:function(e){nyanPlayerReady=true;try{e.target.setVolume(NYAN_VOLUME)}catch(x){}resolve(e.target)},
...
function startNyanMusic(looping){
  ...
  ensureNyanPlayer().then(function(p){
    try{p.setVolume(NYAN_VOLUME);if(looping||p.getCurrentTime()<NYAN_START||p.getCurrentTime()>=NYAN_END)p.seekTo(NYAN_START,true);p.playVideo();startNyanLoopWatch()}catch(e){}
...
function startNeonMusic(reset){
  ...
  try{p.volume=NEON_VOLUME;if(reset)p.currentTime=0;var playPromise=p.play();if(playPromise&&playPromise.catch)playPromise.catch(function(){})}catch(e){}
}
...
function stopMusic(){clearInterval(musicTimer);musicTimer=null;stopArcadeMusic();stopNyanMusic();stopNeonMusic()}
...
function speakText(text,opts){if(!S.voiceOn||!("speechSynthesis"in window))return;opts=opts||{};try{var u=new SpeechSynthesisUtterance(text);u.lang="pt-BR";u.rate=opts.rate||.82;u.pitch=opts.pitch||.96;var v=getPtVoice();if(v)u.voice=v;var duckedArcade=false,duckedNyan=false,duckedNeon=false;if(P.theme==="arcade"&&arcadePlayerReady&&arcadePlayer&&S.musicOn){try{arcadePlayer.setVolume(9);duckedArcade=true}catch(e){}}if(P.theme==="nyan"&&nyanPlayerReady&&nyanPlayer&&S.musicOn){try{nyanPlayer.setVolume(8);duckedNyan=true}catch(e){}}if(P.theme==="neon"&&neonAudio&&S.musicOn){try{neonAudio.volume=.08;duckedNeon=true}catch(e){}}function restore(){if(duckedArcade&&arcadePlayerReady&&arcadePlayer)try{arcadePlayer.setVolume(ARCADE_VOLUME)}catch(e){}if(duckedNyan&&nyanPlayerReady&&nyanPlayer)try{nyanPlayer.setVolume(NYAN_VOLUME)}catch(e){}if(duckedNeon&&neonAudio)try{neonAudio.volume=NEON_VOLUME}catch(e){}}u.onend=restore;u.onerror=restore;speechSynthesis.cancel();speechSynthesis.resume();setTimeout(function(){try{speechSynthesis.speak(u)}catch(e){restore()}},60)}catch(e){}}
...
function refreshAudioButtons(){E.voiceToggle.textContent=S.voiceOn?"🔊 Narração":"🔇 Narração";E.soundToggle.textContent=S.soundOn?"🔔 Efeitos":"🔕 Efeitos";E.musicToggle.textContent=S.musicOn?(P.theme==="arcade"?"👾 Música Arcade":P.theme==="nyan"?"🐾 Música Nyan":P.theme==="neon"?"🌃 Música Neon":"🎵 Música"):"🎵 Música off";E.voiceToggle.classList.toggle("voiceOff",!S.voiceOn);E.soundToggle.classList.toggle("voiceOff",!S.soundOn);E.musicToggle.classList.toggle("voiceOff",!S.musicOn)}
...
);conn.on("close",function(){var p=Array.from(S.players.values()).find(function(x){return x.peer===conn.peer});if(p){p.peer="";S.players.set(p.id,p)}S.conns.delete(conn.peer);sync()})}
...
function connectGuestPeer(){var p=new Peer(undefined,{debug:0});S.peer=p;p.on("open",function(){connectToHost()});p.on("error",function(){net("erro");E.status.textContent="Falha de conexão."})}
...
function themedMusicGestureUnlock(){if(S.musicOn&&P.theme==="arcade")startArcadeMusic(false);if(S.musicOn&&P.theme==="nyan")startNyanMusic(false);if(S.musicOn&&P.theme==="neon")startNeonMusic(false);document.removeEventListener("pointerdown",themedMusicGestureUnlock)}
document.addEventListener("pointerdown",themedMusicGestureUnlock,{passive:true});E.voiceToggle.onclick=function(){S.voiceOn=!S.voiceOn;localStorage.setItem("bingoVoice",S.voiceOn?"on":"off");refreshAudioButtons();if(S.voiceOn)speakText("Narração Nerdora ativada.")};E.soundToggle.onclick=function(){S.soundOn=!S.soundOn;localStorage.setItem("bingoSound",S.soundOn?"on":"off");refreshAudioButtons();if(S.soundOn)sfx("mark")};E.musicToggle.onclick=function(){S.musicOn=!S.musicOn;localStorage.setItem("bingoMusic",S.musicOn?"on":"off");refreshAudioButtons();if(S.musicOn){startMusic();if(P.theme==="arcade")toast("👾 Música Arcade ativada.");if(P.theme==="nyan")toast("🐾 Música Nyan ativada.");if(P.theme==="neon")toast("🌃 Música Neon ativada.")}else stopMusic()};E.themeSelect.onchange=function(){setTheme(E.themeSelect.value,true)};
