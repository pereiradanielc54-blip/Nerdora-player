...
var voiceChatEnabled=false,voiceChatMuted=false,voiceStream=null,voiceCalls=new Map(),voiceAudios=new Map(),voicePeerBound=null,voiceMeterCtx=null,voiceMeters=new Map(),voiceSpeakingPeers=new Set(),voiceMeterTimer=null,voiceRetryTimer=null,cardTouchX=0,cardTouchY=0,cardTouchTime=0,cardSwipeSuppressUntil=0;
...
function normalizedCardCount(n){n=Number(n)||1;return n===3||n===5?n:1}
function makeCards(count){count=normalizedCardCount(count);var out=[];for(var i=0;i<count;i++)out.push(makeCard());return out}
function freshMarksByCard(count){count=normalizedCardCount(count);var out=[];for(var i=0;i<count;i++)out.push(new Set([12]));return out}
function marksToArrays(list){return(Array.isArray(list)?list:[]).map(function(s){return Array.from(s instanceof Set?s:new Set(Array.isArray(s)?s:[12]))})}
function normalizeMarksByCard(raw,count){count=normalizedCardCount(count);var out=[];for(var i=0;i<count;i++){var src=Array.isArray(raw)&&Array.isArray(raw[i])?raw[i]:[12],clean=src.filter(function(x){return Number.isInteger(x)&&x>=0&&x<25});if(clean.indexOf(12)<0)clean.push(12);out.push(new Set(clean))}return out}
function validCardsArray(raw,count){return Array.isArray(raw)&&raw.length===count&&raw.every(function(card){return Array.isArray(card)&&card.length===25})}
function syncActiveCard(){var count=normalizedCardCount(S.config.cardCount);if(!validCardsArray(S.cards,count))S.cards=makeCards(count);if(!Array.isArray(S.marksByCard)||S.marksByCard.length!==count)S.marksByCard=freshMarksByCard(count);S.activeCard=((Number(S.activeCard)||0)%count+count)%count;S.card=S.cards[S.activeCard];S.marked=S.marksByCard[S.activeCard];if(!(S.marked instanceof Set)){S.marked=new Set(Array.isArray(S.marked)?S.marked:[12]);S.marksByCard[S.activeCard]=S.marked}S.marked.add(12)}
function setMultiCards(cards,marks,active){var count=normalizedCardCount((cards&&cards.length)||S.config.cardCount);S.config.cardCount=count;S.cards=validCardsArray(cards,count)?cards.map(function(x){return x.slice()}):makeCards(count);S.marksByCard=normalizeMarksByCard(marks,count);S.activeCard=Math.max(0,Math.min(count-1,Number(active)||0));syncActiveCard()}
function localWinningCardIndex(){if(S.spectator||S.winner)return-1;syncActiveCard();for(var i=0;i<S.cards.length;i++){var marks=S.marksByCard[i]||new Set([12]);if(modeComplete(marks,S.config.mode)&&allMarkedCalled(S.cards[i],marks))return i}return-1}
function localNearCardIndex(){if(S.spectator)return-1;syncActiveCard();for(var i=0;i<S.cards.length;i++){var marks=S.marksByCard[i]||new Set([12]);if(nearMode(marks,S.config.mode)&&!modeComplete(marks,S.config.mode))return i}return-1}
function renderCardNav(){if(!E.multiCardNav)return;var count=normalizedCardCount(S.config.cardCount);E.multiCardNav.classList.toggle("hidden",count<=1);E.cardCounter.textContent="Cartela "+(S.activeCard+1)+"/"+count;E.cardDots.innerHTML="";for(var i=0;i<count;i++){var dot=document.createElement("button");dot.type="button";dot.className="cardDot"+(i===S.activeCard?" active":"");var marks=S.marksByCard[i]||new Set([12]);if(nearMode(marks,S.config.mode)&&!modeComplete(marks,S.config.mode))dot.classList.add("near");dot.dataset.index=String(i);dot.setAttribute("aria-label","Abrir cartela "+(i+1));dot.onclick=function(){goToCard(Number(this.dataset.index))};E.cardDots.appendChild(dot)}}
function goToCard(index){var count=normalizedCardCount(S.config.cardCount);if(count<=1||S.cardSwitching)return;index=((Number(index)||0)%count+count)%count;if(index===S.activeCard)return;var forward=(index-S.activeCard+count)%count,backward=(S.activeCard-index+count)%count;switchCard(forward<=backward?forward:-backward)}
function switchCard(delta){var count=normalizedCardCount(S.config.cardCount);if(count<=1||S.cardSwitching||!delta)return;var dir=delta>0?1:-1,next=((S.activeCard+delta)%count+count)%count;if(next===S.activeCard)return;if(G.reduceMotion){S.activeCard=next;syncActiveCard();renderCard();return}S.cardSwitching=true;var out=dir>0?"cardRotateOutLeft":"cardRotateOutRight",inn=dir>0?"cardRotateInRight":"cardRotateInLeft";E.cardCarousel.classList.add(out);setTimeout(function(){S.activeCard=next;syncActiveCard();renderCard();E.cardCarousel.classList.remove(out);E.cardCarousel.classList.add(inn);setTimeout(function(){E.cardCarousel.classList.remove(inn);S.cardSwitching=false},240)},145)}
...
function bingoReady(){return localWinningCardIndex()>=0}
...
function playerNear(p){if(!p||p.spectator)return false;var groups=Array.isArray(p.marksByCard)&&p.marksByCard.length?p.marksByCard:[Array.isArray(p.marked)?p.marked:[12]];return groups.some(function(m){var set=new Set(m);return nearMode(set,S.config.mode)&&!modeComplete(set,S.config.mode)})}
function readConfig(){return{roomName:(E.roomName.value.trim()||("Sala de "+(clean()||"Nerdora"))).slice(0,28),public:!!E.publicRoom.checked,mode:E.modeSelect.value||"line",speed:Math.max(4000,Math.min(12000,Number(E.speedSelect.value)||7000)),maxPlayers:Math.max(2,Math.min(8,Number(E.maxPlayers.value)||6)),cardCount:normalizedCardCount(E.cardCountSelect?E.cardCountSelect.value:1)}}
...
function renderCard(){
  syncActiveCard();E.card.innerHTML="";E.card.dataset.cardIndex=String(S.activeCard);E.card.classList.toggle("spectatorCard",S.spectator);
  S.card.forEach(function(n,i){var marked=S.marked.has(i),called=i===12||S.called.indexOf(n)>=0,b=document.createElement("button");b.className="cell"+(marked?" marked":"")+(marked&&!called?" pending":"")+(i===12?" free":"");b.disabled=S.spectator;if(i===12)b.innerHTML='<span class="nerdoraMark" aria-label="Nerdora">N</span>';else b.textContent=n;b.onclick=function(){if(Date.now()<cardSwipeSuppressUntil||i===12||S.spectator)return;S.marked.has(i)?S.marked.delete(i):S.marked.add(i);S.marksByCard[S.activeCard]=S.marked;sfx("mark");renderCard();renderBingoState();sendMarks()};E.card.appendChild(b)});
  renderCardNav();renderBingoState();renderNear()
}
function renderNear(){var idx=localNearCardIndex(),near=idx>=0;E.nearBingo.classList.toggle("hidden",!near);if(near)E.nearBingo.textContent=(S.cards.length>1?"🔥 Cartela "+(idx+1)+" está a 1 para ":"🔥 Falta 1 para ")+modeLabel(S.config.mode)+"!";renderNyan()}
...
function historyCapacity(){
  ...
  var rect=E.history.getBoundingClientRect(),style=getComputedStyle(E.history),
      gap=parseFloat(style.rowGap||style.gap)||6,
      cols=Math.max(1,Math.floor((rect.width+gap)/40)),
      rows=Math.max(1,Math.floor((rect.height+gap)/40));
  return Math.max(cols,Math.min(75,cols*rows))
}
function renderHistory(){
  ...
  var maxVisible=historyCapacity(),visible=S.called.slice(0,maxVisible);
  visible.forEach(function(n,idx){var d=document.createElement("div");d.className="hball "+tone(n)+(idx===0?" current":"");d.innerHTML='<span class="hletter">'+letter(n)+'</span><span class="hnum">'+n+'</span>';E.history.appendChild(d)});
  E.history.dataset.visible=String(visible.length);E.history.dataset.total=String(S.called.length);
  E.last.textContent=latest?label(latest):"—";E.last.className="ball"+(latest?" "+tone(latest):"");
  if(latest){void E.last.offsetWidth;E.last.classList.add("ballPop");speakBall(latest)}
  E.count.textContent=S.called.length+" de 75 bolas";E.roundProgressFill.style.width=Math.min(100,S.called.length/75*100)+"%";
  E.title.textContent=S.winner?"Partida encerrada":latest?"Última bola: "+label(latest):"Aguardando início";
  renderCard();renderAutoState();renderEvent()
}
...
function renderOpponents(){
  if(!G.showOpponents){E.opponentsArea.classList.add("hidden");E.opponentsGrid.innerHTML="";return}
...
function awardCardScore(p,cardIndex,marks){
  if(!p||p.spectator||!Array.isArray(p.cards)||!Array.isArray(p.cards[cardIndex]))return;
  p.score=p.score||0;if(!Array.isArray(p.scoredMarksByCard))p.scoredMarksByCard=[];
  var scored=new Set(Array.isArray(p.scoredMarksByCard[cardIndex])?p.scoredMarksByCard[cardIndex]:[]);
  (Array.isArray(marks)?marks:Array.from(marks||[])).forEach(function(i){if(i===12||scored.has(i))return;var n=p.cards[cardIndex][i];if(n&&S.called.indexOf(n)>=0){scored.add(i);p.score+=2}});
  p.scoredMarksByCard[cardIndex]=Array.from(scored)
}
function awardAllCardScores(p){var groups=Array.isArray(p.marksByCard)?p.marksByCard:[p.marked||[12]];groups.forEach(function(m,i){awardCardScore(p,i,m)})}
function scoreCalledMarks(){if(!S.host)return;S.players.forEach(function(p){awardAllCardScores(p);S.players.set(p.id,p)})}
function sendMarks(){
  syncActiveCard();var groups=marksToArrays(S.marksByCard),marks=groups[S.activeCard]||[12];
  if(S.host){var me=S.players.get(S.id);if(me){me.cards=S.cards.map(function(x){return x.slice()});me.marksByCard=groups;me.marked=marks;me.activeCard=S.activeCard;awardAllCardScores(me);me.near=playerNear(me);S.players.set(S.id,me);sync()}}
  else if(S.hostConn&&S.hostConn.open)S.hostConn.send({type:"mark",id:S.id,marked:marks,marksByCard:groups,activeCard:S.activeCard})
}
function publicPlayers(){return Array.from(S.players.values()).map(function(p){return{id:p.id,name:p.name,host:!!p.host,peer:p.peer||"",marked:Array.isArray(p.marked)?p.marked:[12],marksByCard:Array.isArray(p.marksByCard)?p.marksByCard:[[12]],activeCard:Number(p.activeCard)||0,cardCount:Array.isArray(p.marksByCard)?p.marksByCard.length:1,ready:!!p.ready,score:p.score||0,wins:p.wins||0,spectator:!!p.spectator,near:!!p.near}})}
...
function finishRound(winnerId,cardIndex){
  if(!S.host||S.winner)return;var w=S.players.get(winnerId);if(!w)return;
  var count=normalizedCardCount(S.config.cardCount),ci=Math.max(0,Math.min(count-1,Number(cardIndex)||0)),
      groups=Array.isArray(w.marksByCard)&&w.marksByCard.length?w.marksByCard:[w.marked||[12]],
      winMarks=new Set(groups[ci]||[12]);
  var bonus=Math.max(0,75-S.called.length)*2;S.players.forEach(function(p){if(!p.spectator)p.score=(p.score||0)+20});w.score=(w.score||0)+100+bonus;w.wins=(w.wins||0)+1;S.players.set(w.id,w);
  var pat=patternName(winMarks,S.config.mode);if(count>1)pat="Cartela "+(ci+1)+" • "+pat;
  S.winner={id:w.id,name:w.name,gain:120+bonus,pattern:pat,cardIndex:ci};S.matchHistory.push({winner:w.name,calls:S.called.length,mode:S.config.mode,round:S.round,card:ci+1,cardCount:count});S.matchHistory=S.matchHistory.slice(-8);S.replayVotes=new Set();S.autoRunning=false;clearInterval(S.autoTimer);S.autoTimer=null;S.nextDrawAt=0;lobbyUnregister();sync()
}
...
function restartRound(){
  if(!S.host)return;clearInterval(S.autoTimer);S.autoTimer=null;S.autoRunning=false;S.nextDrawAt=0;S.called=[];S.bag=makeBag();S.winner=null;S.event=null;S.roundStarted=false;S.drawing=false;S.countdownStarting=false;S.round++;S.replayVotes=new Set();
  var count=normalizedCardCount(S.config.cardCount);setMultiCards(makeCards(count),null,0);
  var me=S.players.get(S.id);if(me){me.cards=S.cards.map(function(x){return x.slice()});me.marksByCard=marksToArrays(S.marksByCard);me.card=S.card;me.marked=[12];me.activeCard=0;me.scoredMarksByCard=[];me.ready=true;me.spectator=false;me.near=false;S.players.set(S.id,me)}
  var deliveries=[];S.players.forEach(function(p){if(p.id===S.id)return;p.cards=makeCards(count);p.marksByCard=marksToArrays(freshMarksByCard(count));p.card=p.cards[0];p.marked=[12];p.activeCard=0;p.scoredMarksByCard=[];p.ready=false;p.spectator=false;p.near=false;deliveries.push({peer:p.peer,cards:p.cards,marksByCard:p.marksByCard})});
  var packetBase={called:[],calledOrder:"newest-first",winner:null,players:publicPlayers(),autoRunning:false,nextDrawAt:0,roundStarted:false,config:S.config,event:null,matchHistory:S.matchHistory,round:S.round,replayVotes:0,chat:S.chat.slice(-6)};
  deliveries.forEach(function(x){var conn=S.conns.get(x.peer);if(conn&&conn.open)conn.send(Object.assign({type:"new_round",cards:x.cards,marksByCard:x.marksByCard,activeCard:0},packetBase))});
  resetRoundUi();E.status.textContent="Nova partida pronta. Todos receberam "+count+(count===1?" cartela.":" cartelas.");lobbyRegisterNow();sync();toast("Nova rodada criada com "+count+(count===1?" cartela.":" cartelas por jogador."))
}
...
function roomPacketForWelcome(p){return Object.assign(statePacket(),{type:"welcome",cards:p&&Array.isArray(p.cards)?p.cards:[],marksByCard:p&&Array.isArray(p.marksByCard)?p.marksByCard:[],activeCard:p?Number(p.activeCard)||0:0})}
function hostConn(conn){
  conn.on("open",function(){S.conns.set(conn.peer,conn)});
  conn.on("data",function(d){
    if(!d||typeof d!=="object")return;
    if(d.type==="join"){
      var id=String(d.id||rid()).slice(0,24),nm=String(d.name||"Jogador").slice(0,24),old=S.players.get(id),
          active=Array.from(S.players.values()).filter(function(p){return!p.spectator}).length,count=normalizedCardCount(S.config.cardCount);
      if(!old&&!S.roundStarted&&active>=S.config.maxPlayers){conn.send({type:"reject_join",reason:"Sala lotada."});return}
      var spectator=old?!!old.spectator:!!S.roundStarted,cards,groups,activeCard=0;
      if(old&&validCardsArray(old.cards,count)){cards=old.cards;groups=Array.isArray(old.marksByCard)?old.marksByCard:marksToArrays(freshMarksByCard(count));activeCard=Number(old.activeCard)||0}
      else if(old&&validCardsArray(d.cards,count)){cards=d.cards.map(function(x){return x.slice()});groups=marksToArrays(normalizeMarksByCard(d.marksByCard,count));activeCard=Number(d.activeCard)||0}
      else{cards=makeCards(count);groups=marksToArrays(freshMarksByCard(count))}
      var p={id:id,name:nm,host:false,peer:conn.peer,cards:cards,marksByCard:groups,activeCard:Math.max(0,Math.min(count-1,activeCard)),card:cards[0],marked:groups[0]||[12],ready:old?!!old.ready:false,score:old?old.score||0:0,wins:old?old.wins||0:0,scoredMarksByCard:old&&Array.isArray(old.scoredMarksByCard)?old.scoredMarksByCard:[],spectator:spectator,near:false};
      p.near=playerNear(p);S.players.set(id,p);conn.send(roomPacketForWelcome(p));sync()
    }
    if(d.type==="mark"){
      var p1=S.players.get(d.id);if(p1){var count1=normalizedCardCount(S.config.cardCount),groups1=marksToArrays(normalizeMarksByCard(d.marksByCard,count1)),ai=Math.max(0,Math.min(count1-1,Number(d.activeCard)||0));p1.marksByCard=groups1;p1.activeCard=ai;p1.marked=groups1[ai]||[12];p1.card=Array.isArray(p1.cards)&&p1.cards[ai]?p1.cards[ai]:p1.card;awardAllCardScores(p1);p1.near=playerNear(p1);S.players.set(p1.id,p1);sync()}
    }
    if(d.type==="ready"){var p2=S.players.get(d.id);if(p2&&!S.roundStarted&&!p2.spectator){p2.ready=!!d.ready;S.players.set(p2.id,p2);sync()}}
    if(d.type==="claim"){
      var p3=S.players.get(d.id),count3=normalizedCardCount(S.config.cardCount),ci=Math.max(0,Math.min(count3-1,Number(d.cardIndex)||0)),marks3=new Set(Array.isArray(d.marked)?d.marked:[]);
      if(p3&&!S.winner&&Array.isArray(p3.cards)&&p3.cards[ci]&&valid(p3.cards[ci],marks3))finishRound(p3.id,ci);else conn.send({type:"reject"})
    }
...
function clientMsg(d){
  ...
  if(d.type==="welcome"||d.type==="state"){
    if(d.type==="welcome"){
      ...
      var count=normalizedCardCount((d.config||{}).cardCount||((d.cards||[]).length)||1);
      S.config=Object.assign(S.config,d.config||{});
      if(validCardsArray(d.cards,count))setMultiCards(d.cards,d.marksByCard,d.activeCard||0)
    }
    applyState(d)
  }
  if(d.type==="new_round"){
    var count2=normalizedCardCount((d.config||S.config).cardCount||((d.cards||[]).length)||1);S.config=Object.assign(S.config,d.config||{});
    setMultiCards(validCardsArray(d.cards,count2)?d.cards:makeCards(count2),d.marksByCard,0);S.spectator=false;resetRoundUi();applyState(d);toast("Nova rodada! Você recebeu "+count2+(count2===1?" cartela.":" cartelas."))
  }
...
function createRoom(){
  var nm=clean();if(!nm){toast("Digite seu nome para criar a sala.");E.name.focus();return}getAudio();localStorage.setItem(NAME_KEY,nm);S.host=true;S.name=nm;S.code=code();S.id="host-"+rid();S.config=readConfig();S.called=[];S.bag=makeBag();S.winner=null;S.roundStarted=false;S.round=1;S.matchHistory=[];S.chat=[];S.replayVotes=new Set();
  var count=normalizedCardCount(S.config.cardCount);setMultiCards(makeCards(count),null,0);
  S.players=new Map([[S.id,{id:S.id,name:nm,host:true,peer:"",cards:S.cards.map(function(x){return x.slice()}),marksByCard:marksToArrays(S.marksByCard),activeCard:0,card:S.card,marked:[12],ready:true,score:0,wins:0,scoredMarksByCard:[],spectator:false,near:false}]]);
  gameUI();E.status.textContent="Sala criada • aguardando jogadores entrarem.";net("abrindo sala");renderWaitingRoom();toast("Sala criada com "+count+(count===1?" cartela por jogador.":" cartelas por jogador."));createPeerHost()
}
function joinRoom(){
  var nm=clean(),cd=E.room.value.trim().toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,6);if(!nm){toast("Digite seu nome.");return}if(cd.length!==6){toast("Digite o código de 6 caracteres.");return}getAudio();localStorage.setItem(NAME_KEY,nm);S.host=false;S.name=nm;S.code=cd;S.id="p-"+rid();S.called=[];S.winner=null;S.roundStarted=false;S.config.cardCount=1;setMultiCards([makeCard()],null,0);S.players=new Map([[S.id,{id:S.id,name:nm,host:false,marked:[12],marksByCard:[[12]],activeCard:0,ready:false,score:0,wins:0,spectator:false}]]);gameUI();net("conectando");connectGuestPeer()
}
...
if(E.tauntToggle)E.tauntToggle.onclick=function(ev){ev.stopPropagation();E.tauntMenu.classList.toggle("hidden")};
document.addEventListener("click",function(ev){if(E.tauntMenu&&!E.tauntMenu.classList.contains("hidden")&&!E.tauntMenu.contains(ev.target)&&ev.target!==E.tauntToggle)E.tauntMenu.classList.add("hidden")});
