...
object LibraryStore {
    ...
    fun replaceFilePathReferences(context: Context, oldPath: String, newPath: String) {
        if (oldPath.isBlank() || newPath.isBlank() || oldPath == newPath) return
        val oldKey = fileKey(oldPath)
        val newKey = fileKey(newPath)

        val favorites = favoriteIds(context).toMutableSet()
        if (favorites.remove(oldKey)) favorites += newKey

        val recents = recentIds(context).map { if (it == oldKey) newKey else it }.distinct()
        prefs(context).edit()
            .putStringSet(FAVORITES, favorites)
            .putString(RECENTS, JSONArray(recents).toString())
            .apply()
    }

    fun removeFilePathReferences(context: Context, path: String) {
        if (path.isBlank()) return
        val key = fileKey(path)
        val favorites = favoriteIds(context).toMutableSet().apply { remove(key) }
        val recents = recentIds(context).filterNot { it == key }
        prefs(context).edit()
            .putStringSet(FAVORITES, favorites)
            .putString(RECENTS, JSONArray(recents).toString())
            .apply()
